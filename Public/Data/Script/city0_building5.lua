...
-- ½»¹ÙÆ±µÄ½±Àø±ÈÀý
x805012_g_GuildRate = 1.00		-- °ïÅÉ»ñµÃµÄ×Ê½ð»Ø±¨100%
x805012_g_PlayerRate = 0.20		-- ¸öÈË»ñµÃµÄ×Ê½ð»Ø±¨20%

-- Ã¿¸öÍæ¼ÒÒ»Ìì¿ÉÒÔÁìÈ¡ÒøÆ±µÄ´ÎÊý
x805012_g_TicketTakeTimes = 8

-- Ã¿¸ö°ïÅÉÒ»Ìì¿ÉÒÔÁìÈ¡ÒøÆ±µÄ´ÎÊý»ùÊý
x805012_g_BaseTotalTicketTakeTimes = 200

-- °ïÅÉÃ¿ÉýÒ»¼¶¿ÉÒÔÔö¼ÓµÄÒøÆ±ÁìÈ¡´ÎÊý
...
function x805012_OnDefaultEvent( sceneId, selfId,targetId )
	...
	-- x805012_g_RandTN = 100000 - GetTodayWeek()*GetMinute()
	x805012_g_RandTN = 5
	...
	if(guildid ~= cityguildid) then
		BeginEvent(sceneId)
			...
			AddNumText( sceneId, x805012_g_scriptId, "CØa hàng thß½ng nhân",7,x805012_g_RandTN)
		EndEvent(sceneId)
	...
	BeginEvent(sceneId)
		...
		--AddNumText( sceneId, x805012_g_scriptId, "CØa hàng thß½ng nhân",7,1)
		AddNumText( sceneId, x805012_g_scriptId, "Gi¾i thi®u ti«n trang", 11, 4 )
		AddNumText( sceneId, x805012_g_scriptId, "CØa hàng thß½ng nhân",7,x805012_g_RandTN)
		AddNumText( sceneId, x805012_g_scriptId, "Con ðß¶ng thß½ng nghi®p", 12, 6 )
		...
		if x805012_IsManager(Guildpos)==1 then
			--¹ÜÀíÕß¸£Àû
			AddNumText( sceneId, x805012_g_scriptId, "Nh§n Bang phái ngß¶i quän lý phúc lþi", 7,  9)
			AddNumText( sceneId, x805012_g_scriptId, "Gi¾i thi®u v« Bang phái phúc lþi", 11, 10 )
		end
...
function x805012_DrawPay( sceneId, selfId  )
	...
	if GetLevel( sceneId, selfId ) < 40 then
		msg = format("Các hÕ không ðü c¤p 40, vì v§y không th¬ lînh ti«n lß½ng.");
	...
	if nTimeDelta < 7*24*60*60 then
		msg = format("Th¶i gian vào bang cüa các hÕ chßa ðü mµt tu¥n, không th¬ lînh ti«n lß½ng.");
		x805012_NotifyTips(sceneId, selfId, msg);
		return 0;
	end
...
function x805012_DrawManagerBonus( sceneId, selfId  )
	...
	if CityGetMaintainStatus(sceneId, selfId, sceneId) == 1  then
		msg = format("Bang hµi bây gi¶ tài chính thiªu høt, m²i mµt phân ti«n ð«u phäi dùng mµt cách th§t tiªt ki®m. Phúc lþi ngß¶i quän lý phäi tÕm d×ng phát cho r°i.");
		x805012_NotifyTips(sceneId, selfId, msg);
		return 0;
	end
	
	if GetTodayWeek() ~= 0 then
	    msg = format("Còn chßa t¾i t¾i lúc phát phúc lþi cho ngß¶i quän lý, không nên s¯t ruµt!");
		x805012_NotifyTips(sceneId, selfId, msg);
		return 0;
	end
	...
	if GetFullExp(sceneId, selfId) == GetExp(sceneId, selfId) then
	    msg = format("Kinh nghi®m cüa các hÕ ðã ðÕt ðªn thßþng hÕn, bây gi¶ nh§n phúc lþi không ðáng giá r°i. Ði dùng giäm mµt ít r°i tr· lÕi ði!");
		x805012_NotifyTips(sceneId, selfId, msg);
		return 0;
	end
	...
	if nWeekCur ~= nDrawPayTimeLast then
		...
		if GuildMoney < goldGet then
			x805012_NotifyTips(sceneId, selfId, "Bang phái hi®n tài chính không ðü, không th¬ nh§n!");
			return 0;
		end
		CityChangeAttr(sceneId, selfId, GUILD_MONEY, (-1)*goldGet)
		AddMoney( sceneId, selfId, goldGet );
		...
		local  PlayerName=GetName(sceneId,selfId);
		local sMessage = format("@*;SrvMsg;GLD:#YB±n bang [%s] [#{_INFOUSR%s}] #cffff00 nh§n lînh bang hµi quan viên b±ng lµc ðßþc #{_MONEY%d} cùng %d ði¬m kinh nghi®m.", GUILD_POSITION_NAME_TABLE[Guildpos],PlayerName, goldGet, expGet );	
        BroadMsgByChatPipe(sceneId, selfId, sMessage, 6);
	  
        msg = format("Chúc vø cüa ngß½i v¯n là: %s",GUILD_POSITION_NAME_TABLE[Guildpos]);
        x805012_NotifyTips(sceneId, selfId, msg);
				
	else
		msg = format("Ðßþc ngß½i là quan viên, v¯n ðã nh§n qua mµt l¥n phúc lþi ngß¶i quän lý r°i, còn mu¯n nh§n mµt làn næa hay sao?");
		x805012_NotifyTips(sceneId, selfId, msg);
		return 0;
	end
...
function x805012_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	-- if GetNumText() == 5 then
		-- LuaFnSendSystemMail(sceneId,"Admin",GetName(sceneId, selfId))
	-- end
	
	--´¦ÀíÕâ¸ö½¨ÖþÎïµÄÏàÓ¦Éú»î¼¼ÄÜÑ¡Ïî
	...
	elseif GetNumText() == 2 then
		...
		elseif bagpos == -1 then
				...
				if totalTakenTimes > x805012_g_TicketTakeTimes then
					BeginEvent( sceneId )
						AddText( sceneId, "Xin l²i, các hÕ nh§n nhi®m vø thß½ng nhân hôm nay ðã ðßþc 8 l¥n, xin ngày mai quay lÕi." )
					EndEvent( sceneId )
					DispatchMissionTips( sceneId, selfId )
	...
	elseif GetNumText() == 3 then
		...
		if bagpos ~= -1	then
			...
			if TicketMoney ~= 0 then
				...
				CityChangeAttr(sceneId, selfId, GUILD_MONEY, TicketMoney * x805012_g_GuildRate*merchandiseDayRate*FatigueRate)	-- °ïÅÉ»ñµÃµÄ×Ê½ð»Ø±¨
				AddMoney(sceneId, selfId, TicketMoney * x805012_g_PlayerRate*merchandiseDayRate)	-- ¸öÈË»ñµÃµÄ×Ê½ð»Ø±¨
				
				--Save Log TN
				-- local DateStr = date("[%Y-%m-%d]-[%H-%M-%S]")
				-- local CharStr = GetHumanGUID(sceneId, selfId)
				-- local CharName = GetName(sceneId, selfId)
				-- local MoneySave = TicketMoney * x805012_g_PlayerRate*merchandiseDayRate;
				
				-- local DateSave = GetTime2Day()
				-- local GuildID = GetHumanGuildID(sceneId, selfId)
				-- if GuildID >= 0 and GuildID <= 9 then
					-- GuildID = "00"..GuildID
				-- elseif GuildID >= 10 and GuildID <= 99 then
					-- GuildID = "0"..GuildID
				-- end
				
				-- local Str = DateStr.."-"..CharStr.."["..CharName.."] - Gold["..MoneySave.."]";
				-- local handle = openfile('Var/ThuongNhan/'..DateSave..'_GUILD_'..GuildID..'.txt', "a+")
				-- if nil ~= handle then
					-- write(handle,Str.."\n")
					-- closefile(handle)
				-- end
				...
				BroadMsgByChatPipe( sceneId, selfId, "@*;SrvMsg;GLD:#Y#{_INFOUSR" .. name .. "}#R Hoàn t¤t nhi®m vø thß½ng nhân, giúp b±n bang tång s¯ ti«n cüa bang hµi lên #{_MONEY"..TicketMoney*x805012_g_GuildRate*merchandiseDayRate*FatigueRate .. "}", 6 )
				--BroadMsgByChatPipe( sceneId, selfId, "22", 6 )
				
				if (merchandiseDayRate ~= 1.0  )then
					Msg2Player(sceneId, selfId, "#RDo hôm nay là ngày thß½ng nhân, vì v§y các hÕ thu ðßþc nhi«u thu nh§p h½n bình thß¶ng.",MSG2PLAYER_PARA);
				end
	...
	elseif GetNumText() == 4 then
		...
		EndEvent(sceneId)
		DispatchEventList( sceneId, selfId, targetId )
	elseif GetNumText() == x805012_g_RandTN then
	...
	elseif GetNumText() == 8 then
		...
		BeginUICommand(sceneId)
			...
			UICommand_AddString(sceneId, "Các hÕ có xác ð¸nh mµt s¯ lßþng nh¤t ð¸nh bang c¯ng, ð¬ nh§n ðßþc ti«n lß½ng trong tu¥n này?");
		EndUICommand(sceneId)
	...
	elseif GetNumText() == 11 then
		...
		else  --°ï»á×Ê½ðÒÑ¾­µ½´ïÉÏÏÞ£¬²»ÐèÒªÔÙ¾èÖúÁË¡£
			BeginEvent(sceneId)
	...
	if index1 < 0 or index1 >= 255 then
		...
		if LuaFnIsItemAvailable( sceneId, selfId, index1) == 1 then
			...
			if(itm_id == x805012_g_OutDateTicketItemIdx) then
				...
				CityChangeAttr(sceneId, selfId, GUILD_MONEY, TicketMoney)	-- °ïÅÉ»ñµÃµÄ×Ê½ð»Ø±¨
				AddMoney(sceneId, selfId, TicketMoney * 0.2)	-- ¸öÈË»ñµÃµÄ×Ê½ð»Ø±¨
				local name = GetName( sceneId, selfId )
...
--**********************************
--Íæ¼ÒÌá½»°ï»á×Ê½ð¾èÖúºóµÄ»Øµ÷º¯Êý
--**********************************
function x805012_PutGuildMoney( sceneId, selfId, money )
	...
	if guildid ~= cityguildid then
		x805012_NotifyTips(sceneId, selfId, "Các hÕ không phäi là thành viên b±n bang!")
		return
	end
	
	if money < x805012_g_GuildMoneyLimit then --²»Ó¦¸Ã×ßµ½ÕâÀï£¬³ý·Ç¿Í»§¶Ë·Ç·¨Êý¾Ý
		x805012_NotifyTips(sceneId, selfId, "Ti«n vàng ðßa vào nhö h½n #{_EXCHG"..x805012_g_GuildMoneyLimit.."}")
		return
	end
	
	--¶Ô²»Æð£¬ÄúÊäÈëµÄ½ð¶î´óÓÚÄúËùÐ¯´øµÄ½ð¶î¡£
	...
	if jzCost == -1 then
		x805012_NotifyTips(sceneId, selfId, "Kh¤u tr× ti«n tài th¤t bÕi!")
		return
	end
	--Èç¹ûÖ»¿Û³ý½ð±Ò
	if jzCost == 0 then
		...
		x805012_NotifyTips(sceneId, selfId, "Sau khi kh¤u tr× thuª, tài chính Bang hµi tång trß·ng thñc tª #{_MONEY"..addmoney.."}!")
		local name = GetName( sceneId, selfId )
