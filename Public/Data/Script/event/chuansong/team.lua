				...
				for	i = 0, followMemberCount - 1 do
					...
					if followMembers[i] and followMembers[i] ~= -1 then
						...
						if followMembers[i] ~= selfId and IsHaveMission(sceneId, followMembers[i], 4021) > 0 then
							x400900_NotifyFailTips(sceneId, selfId, "Các hÕ và bÕn bè vçn · trong trÕng thái Tào v§n, không th¬ d¸ch chuy¬n.")
							return
						end
						
						if followMembers[i] ~= selfId and GetItemCount(sceneId, followMembers[i], x400900_g_Yinpiao) >=1 then
							x400900_NotifyFailTips(sceneId, selfId, "Các hÕ và bÕn bè vçn · trong trÕng thái thß½ng nhân, không th¬ d¸ch chuy¬n.")
							return
						end
						...
						if memberHasDRideFlag and memberHasDRideFlag == 1 then
							...
							if memberIsDRideMountOwner and memberIsDRideMountOwner == 1 then
								...
								x400900_NotifyFailTips(sceneId, selfId, "2 ngß¶i cßÞi không th¬ cùng t°n tÕi v¾i t± ðµi.")
								return
							end
						end
						...
						x400900_NotifyFailTips(sceneId, selfId, "Các hÕ còn có bÕn bè ði theo không th¬ ðªn ðßþc cänh này, tÕm th¶i không th¬ r¶i khöi dây.")
						return
					end
				end
			else
			...
			if isDRideMountOwner and isDRideMountOwner == 1 then
				...
				if drideTargetID and drideTargetID ~= -1 then
					...
					if drideTargetID ~= selfId and IsHaveMission(sceneId, drideTargetID, 4021) > 0 then
						x400900_NotifyFailTips(sceneId, selfId, "Các hÕ ho£c trong nhóm cüa các hÕ có ð¯i phß½ng 2 ngß¶i cßÞi · trÕng thái Tào v§n, không th¬ d¸ch chuy¬n.")
						return
					end
					
					if drideTargetID ~= selfId and GetItemCount(sceneId, drideTargetID, x400900_g_Yinpiao) >=1 then
						x400900_NotifyFailTips(sceneId, selfId, "Các hÕ ho£c trong tuÏ tùng cüa các hÕ có ð¯i phß½ng 2 ngß¶i cßÞi · trÕng thái bào thß½ng, không th¬ d¸ch chuy¬n.")
						return
					end
...
function x400900_TransferFuncFromNpc( sceneId, selfId, newSceneId, posX, posY, minLevel, maxLevel )
	if IsHaveMission( sceneId, selfId, 4021 ) > 0 then
		BeginEvent( sceneId )
			local strText = "Các hÕ ðang · trÕng thái Tào v§n, không th¬ sØ døng chÑc nång chuyên ch·."
...
function x400900_CheckChangeScene(sceneId, selfId, newSceneId, posX, posY, minLevel, maxLevel)
	if not sceneId or not selfId or not newSceneId or not posX or not posY then
		return 0, "Ð« ngh¸ d¸ch chuy¬n không thñc hi®n ðßþc, không th¬ d¸ch chuy¬n.", "Ð« ngh¸ d¸ch chuy¬n không thñc hi®n ðßþc, không th¬ d¸ch chuy¬n.";
	end
	...
	if not livingFlag or livingFlag ~= 1 then
		return 0, "Các hÕ ðã chªt, không th¬ d¸ch chuy¬n.", GetName(sceneId, selfId).." ðã chªt, không th¬ d¸ch chuy¬n.";
	end
	
	if minLevel and selfLevel < minLevel then
		return 0, "C¤p b§c cüa các hÕ không ðü "..tostring(minLevel)..", không th¬ d¸ch chuy¬n.", GetName(sceneId, selfId).." c¤p b§c không ðü "..tostring(minLevel)..", không th¬ d¸ch chuy¬n."; 
	end
	
	if maxLevel and selfLevel >= maxLevel then
		return 0, "C¤p b§c cüa các hÕ c¥n nhö h½n "..tostring(maxLevel)..", m¾i có th¬ d¸ch chuy¬n.", GetName(sceneId, selfId).." c¤p b§c c¥n nhö h½n "..tostring(maxLevel)..", m¾i có th¬ d¸ch chuy¬n.";
	end
