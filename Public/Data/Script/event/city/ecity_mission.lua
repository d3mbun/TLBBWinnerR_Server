...
-- ²¿·ÖÈÎÎñ²ÎÊı
x600001_g_IsMissionOkFail = 0						-- Nhi®m vø hoàn thành±ê¼Ç
x600001_g_MissionParam_SubId = 1					-- ×ÓÈÎÎñ½Å±¾ºÅ´æ·ÅÎ»ÖÃ
x600001_g_TransScript = 400900

-- Ã¿ÌìÍæ¼ÒÄÜ×ötoÕ ğµ ×î¶àtoÕ ğµ °ïÅÉS¯ l¥n nhi®m vø 
...
x600001_g_HighRate_Param = 0.8;			-- °´70c¤pÍæ¼ÒtoÕ ğµ 10%µ÷³É8%
x600001_g_LowRate_Param = 0.875;		-- °´70c¤pÍæ¼ÒtoÕ ğµ 40%µ÷³É35%
--µÍc¤pÈÛÁ¶·û
...
function x600001_DoDefaultEvent( sceneId, selfId, targetId, missionId, subMissionScriptId )
	...
	if IsHaveMission( sceneId, selfId, missionId ) > 0 then
		...
		if not efsRet or efsRet == 1 then
			x600001_NotifyFailTips( sceneId, selfId, "Ğang m®t, không th¬ nh§n nhi®m vø." )
			return
		end

		-- Èç¹ûÎ´½Ó¸ÃÈÎÎñ
		if LuaFnGetTaskItemBagSpace( sceneId, selfId ) < 1 then
			x600001_NotifyFailTips( sceneId, selfId, "Nhi®m vø này c¥n ít nh¤t mµt không gian v§t ph¦m nhi®m vø." )
			return
		end
...
--**********************************
-- ¹©¼ì²âTiªp thøÌõ¼ştoÕ ğµ ÈÎÎñº¯Êıµ÷ÓÃ
--**********************************
function x600001_DoCheckAccept( sceneId, selfId, missionId, timeParam )
	...
	-- ¼ì²âĞúng·ñÈÎÎñÒÑ¾­´ïµ½ 20 cái,Èç¹ûµ½ÁË,Ôò²»ÄÜ½Ó
	...
	local iTime = GetMissionData( sceneId, selfId, timeParam )					-- ÉÏmµt ´Î·ÅÆúÈÎÎñtoÕ ğµ Ê±¼ä(mµt ¿ÌÖÖ)
	--local CurTime = GetHourTime()												-- µ±Ç°Ê±¼ä(mµt ¿ÌÖÓ)
	local CurTime = GetQuarterTime()												-- µ±Ç°Ê±¼ä(mµt ¿ÌÖÓ)
...
function x600001_DoAccept( sceneId, selfId, scriptId, missionId, roundParam )
	--¼ÓÈëÈÎÎñµ½Íæ¼ÒÁĞ±í
	local misIndex = GetMissionIndexByID( sceneId, selfId, missionId )				--  ği¬mµ½ÈÎÎñtoÕ ğµ ĞòÁĞºÅ
	SetMissionByIndex( sceneId, selfId, misIndex, x600001_g_IsMissionOkFail, 0 )	-- ¸ù¾İĞòÁĞºÅ°ÑÈÎÎñ±äÁ¿toÕ ğµ µÚ0Î»ÖÃ0 (Nhi®m vø hoàn thànhÇé¿ö)
	SetMissionByIndex( sceneId, selfId, misIndex, x600001_g_MissionParam_SubId, scriptId )	-- ¸ù¾İĞòÁĞºÅ°ÑÈÎÎñ±äÁ¿toÕ ğµ µÚ1Î»ÖÃÎªÈÎÎñ½Å±¾ºÅ
...
--**********************************
-- ·ÅÆúËùÓĞThành ph¯ ÈÎÎñ
--**********************************
function x600001_AbandonCityMissions( sceneId, selfId )
	local missionList1 = { 1112 }	-- ÓÅÏÈc¤p×î¸ßtoÕ ğµ 
	local missionList2 = { 1105, 1106, 1107, 1108, 1110, 1109, 1111, 1113, 1140, 1141, 1142 }	-- ´ÎÓÅÏÈc¤ptoÕ ğµ 
...
function x600001_DoSubmit( sceneId, selfId, missionId, roundParam )
	...
	MissionName[MD_CITY_ENGINEERING_ROUND] = "Công trình"									-- ¹¤³Ì
	MissionName[MD_CITY_DEVELOPMENT_ROUND] = "Phát tri¬n"									-- ·¢Õ¹
	MissionName[MD_CITY_SCITECH_ROUND] = "Công ngh®"		   								-- ¿Æ¼¼
	MissionName[MD_CITY_MARKET_ROUND] = "Th¸ t§p"		   								-- ÊĞ¼¯
	MissionName[MD_CITY_EXPAND_ROUND] = "M· rµng"		   								-- À©ÕÅ
	MissionName[MD_CITY_MILITARY_ROUND] = "Qu¯c phòng"									-- ¹ú·À
	...
	if MissionName[roundParam] then
		...
		BeginEvent( sceneId )
			AddText( sceneId, "#{BHRW_081124_01} Nhi®m vø "..MissionName[roundParam].." "..MissionRound.." #{BHRW_081124_02} "..nCount.."/"..x600001_g_CountLimiti.." #{BHRW_081124_03}")
		EndEvent( sceneId )
	...
	if MissionName[roundParam] and x600001_g_MaxRound <= MissionRound then
		local str = format( "@*;SrvMsg;GLD: #YSau khi #{_INFOUSR%s} #Ykhông ng×ng n± lñc, cu¯i cùng ğã hoàn thành #R%d#Y vòng %s nhi®m vø.",
...
function x600001_BackToCity( sceneId, selfId, missionId, xPos, zPos )
	if IsHaveMission( sceneId, selfId, missionId ) > 0 then							--Èç¹û½øÈë¸±±¾Ç°É¾³ıÈÎÎñ,ÔòÖ±½Ó´«ËÍ»Ø
		local misIndex = GetMissionIndexByID( sceneId, selfId, missionId )
		if GetMissionParam( sceneId, selfId, misIndex, x600001_g_IsMissionOkFail ) ~= 1 then
			x600001_NotifyFailTips( sceneId, selfId, "Nhi®m vø th¤t bÕi!" )
...
--**********************************
-- Kinh nghi®m½±ÀøÖµ¼ÆËã
-- Ëã·¨: µ±Ç°ÈÎÎñÖÖÀà±ÈÀı*(Íæ¼ÒµÈc¤p^±ÈÀı)*»·Êı+»ùÊı
--**********************************
function x600001_CalcExpBonus( sceneId, selfId, roundParam )
	...
	local Level = GetLevel( sceneId, selfId )										-- Íæ¼ÒµÈc¤p
	local l_Exp = 1.75																-- ±ÈÀı
	local MissionRound = GetMissionData( sceneId, selfId, roundParam )				-- »·Êı
	local BaseExp = 0																-- »ùÊı

	local Exp = TypeScale * ( Level ^ l_Exp ) * MissionRound + BaseExp				-- µÈc¤p + »·Êıº¯Êı,ÊÜKinh nghi®mµ÷½Ú³£ÊıtoÕ ğµ Ó°Ïì
...
function x600001_RandomItemAward( sceneId, selfId, roundParam, missionItemIdx )
	...
	if MissionRound >= x600001_g_MaxRound then												-- ¶¥c¤pÓĞ½±ÎïÆ·¼¸ÂÊ
		...
		if itemSN > -1 then
			...
			if bagpos and bagpos ~= -1 then
				...
				if MissionName[roundParam] then
					...
					local str = format( "@*;SrvMsg;GLD:#Y#{_INFOUSR%s}#cffff00không nh×ng n± lñc, cu¯i cùng ğã hoàn thành 10 vòng %s nhi®m vø, thu ğßşc %s r°i #{_INFOMSG%s}.",
...
function x600001_RandomItemMeltingAward( sceneId, selfId, roundParam )
	...
	if MissionRound >= x600001_g_MaxRound then												-- ¶¥c¤pÓĞ½±ÎïÆ·¼¸ÂÊ
		...
		local iPlayerLv = GetLevel(sceneId,selfId)											-- ÅĞ¶ÏÍæ¼ÒµÈc¤p
		...
		if (rand < rateHigh) then																				-- Ëæ»úµ½¸ßc¤pÈÛÁ¶·û
				itemSN = 30900057
		else
				if (rand < rateLow) then																		-- Ëæ»úµ½µÍc¤pÈÛÁ¶·û
					itemSN = 30900056 
				else
					return																										-- É¶¶¼Ã»Ëæ»úµ½
				end
		...
		if itemSN > -1 then
			...
			if bagpos and bagpos ~= -1 then
				...
				if MissionName[roundParam] then
					...
					if (itemSN == 30900057) then	--¸ßc¤p¹«¸æ
					...
					if (itemSN == 30900056) then	--µÍc¤p¹«¸æ
...
function x600001_CanDoMisToDay(sceneId, selfId)
	...
	if nCount >= x600001_g_CountLimiti  then
		BeginEvent( sceneId )
			AddText( sceneId, "  Xin l²i, nhi®m vø hôm nay cüa các hÕ ğã hoàn thành 50 l¥n, ch¡c ch¡n r¤t m®t r°i, hay là ği nghï mµt lúc r°i mai quay lÕi tìm tÕi hÕ." )
		EndEvent( sceneId )
...
--**********************************
-- Íæ¼ÒÍê³Émµt ´ÎÈÎÎñºó,¶¼¼ÇÂ¼mµt ÏÂ´ÎÊıºÍÊ±¼ä
--**********************************
