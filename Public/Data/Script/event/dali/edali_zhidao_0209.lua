...
x210209_g_Name_9="Ğàm Ğài TØ Vû"
x210209_g_Name_10="Mµ Dung Truy«n"
...
{name="Ğàm Ğài TØ Vû",	skill="H÷c Nhiên Thiêu Hãm Tinh"},
{name="Mµ Dung Truy«n",	skill="H÷c Tinh Tuy«n Phän Kích"},
}
-->>end
--MisDescEnd
--**********************************
...
function x210209_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	if	GetName(sceneId,targetId) == x210209_g_XinShouJiNeng[8].name	then
			--ÅĞ¶¨Íæ¼ÒĞúng²»ĞúngÒÑ¾­Ñ§»áÁËCái này ¼¼ÄÜ,»áÁË¾Í²»ÈÃTÕi Ñ§Ï°ÁË
			if  HaveSkill(sceneId, selfId, 248) > 0  then
				return
			end
	...
	if x210209_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		elseif	GetName(sceneId,targetId) == x210209_g_XinShouJiNeng[9].name	then
			...
			BeginEvent(sceneId)
				strText = "Ngß½i ğã h÷c ğßşc kÛ nång m¾i: kÛ nång Tân thü Tiêu Dao: Bùng Cháy CÕm Bçy"
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		--¿Û³ıÈÎÎñÎïÆ·
		...
		elseif	GetName(sceneId,targetId) == x210209_g_XinShouJiNeng[10].name	then
			...
			BeginEvent(sceneId)
				strText = "Ngß½i ğã h÷c ğßşc kÛ nång m¾i: kÛ nång Tân thü Mµ Dung: Tinh Tuy¬n Phän Kích"
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
