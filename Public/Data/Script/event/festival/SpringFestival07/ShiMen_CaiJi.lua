...
-- ÈÎÎñÎÄ±¾ÃèÊö
x050059_g_MissionName = "Môn phái nåm m¾i mua quà tªt"
...
function x050059_GetStrIndexByStrValue(stringV)
	...
	local strText = format("Phäi ðem %s ðång ký · StrList", stringV)
	----PrintStr(strText)
	return 0;
end
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x050059_OnDefaultEvent( sceneId, selfId, targetId,MenPai,MissionID,ScriptID  )	--µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	--¼ì²âÃÅÅÉÐÅÏ¢ÊÇ·ñºÏ·¨
...
--**********************************
-- »ñµÃ½ñÌìµÄÈÎÎñÊýÁ¿ -1±íÊ¾½ñÌìµÄÈÎÎñÒÑÂú¶î
--**********************************
function x050059_GetTaskCount(sceneId,selfId)
	--ÅÐ¶Ï½ñÌìµÄÈÎÎñÁ¿ÊÇ·ñ³¬³ö
		local iTaskCount =0
		local td = GetDayTime()
		...
		if td < LastGetTaskTime then
			iTaskCount = -1
		--µ±Ç°ÈÕÆÚ£¬¼ì²âµ±ÌìµÄÈÎÎñ´ÎÊý
		else
			local TaskCount = GetMissionData(sceneId,selfId,MD_SHIMENCAIJI_COUNT)
			--µ±ÌìµÄÈÎÎñ´ÎÊý´óÓÚ24£¬»¹ÓÐ¿ÉÄÜÊÇ½»ÈÎÎñ£¬ËùÒÔ24ÔÚ´Îº¯ÊýÖÐÈÏÎªÊÇºÏ·¨´ÎÊý
...
function x050059_AcceptTask(sceneId, selfId,targetId,MenPai,MissionID,ScriptID)
	...
	--½ñÌìµÄÈÎÎñ´ÎÊýÒÑÂú£¬ÐÑÄ¿ÌáÊ¾£¬ÍË³ö
...
function x050059_Accept(sceneId, selfId,MissionID,ScriptID)
	...
	Msg2Player(sceneId, selfId, "#YNh§n nhi®m vø: Môn phái nåm m¾i mua quà tªt", MSG2PLAYER_PARA )
	...
	--ÔÚFormatListÖÐµÄÐòºÅ
	SetMissionByIndex(sceneId, selfId, misIndex, x050059_g_StrForePart, iRandom-1)--formet×Ö·û´®Ë÷Òý
	--ÔÚstrListÖÐµÄÐòºÅ
	...
	AddText(sceneId,"Hi®n tÕi l¥n này là  l¥n thÑ "..(floor(iDayCount/x050059_g_Times)+1).." vòng "..iTimes.." nhi®m vø")
...
function x050059_GetItemDetailInfo(itemId)
	...
	if itemId == -1 then
		local strText = "V§t ph¦m không t°n tÕi!!"
		PrintStr(strText)
	end
...
function x050059_OnSubmit_Necessary( sceneId, selfId, targetId)
	...
	--ÉèÖÃµ±ÌìµÄÈÎÎñ×ÜÊý(½ÓÊÜÈÎÎñµÄÊ±ºòÒÑ¾­¼ÇÂ¼ÁË´ÎÊý£¬ËùÒÔ×¢ÊÍÁË´ËÐÐ
...
function x050059_OnAbandon( sceneId, selfId,MissionID)
	...
	if IsHaveMission(sceneId, selfId, MissionID) > 0 then	
	  ...
	  DelMission( sceneId, selfId, MissionID )
		...
		if td > LastGetTaskTime then
			SetMissionData(sceneId, selfId, MD_SHIMENCAIJI_DAY, td)
			SetMissionData(sceneId,selfId,MD_SHIMENCAIJI_COUNT,0)	--µ±Ç°»·ÊýÇå0
		...
		--µ±Ç°ÈÕÆÚ£¬¼ì²âµ±ÌìµÄÈÎÎñ´ÎÊý
	  	...
	  	SetMissionData(sceneId,selfId,MD_SHIMENCAIJI_COUNT,iDayHuan*x050059_g_Times)	--µ±Ç°»·ÊýÇå0
	  	local iAbandonCount = GetMissionData(sceneId,selfId,MD_SHIMENCAIJI_ABANDON)
