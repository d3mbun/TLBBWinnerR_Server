...
--Ç§Ñ°ÈÎÎñÊÂ¼ş½Å±¾
--Ã¿ÖÖ¹ØÏµÈÎÎñÃ¿ÌìÖ»ÄÜ×öÒ»´Î,Ã¿´Î20»·
...
x229024_g_MissionInfo_IDX			= 121	--ÈÎÎñËµÃ÷Ë÷Òı
...
--Ìí¼ÓÌØÊâµÄÒ»ÖÜÁ¬ĞøÇ§Ñ°£¬czf£¬2009.08.07
x229024_g_SpecialBeginTime	= 200130824
x229024_g_SpecialEndTime	= 20130830
--ËùÓµÓĞµÄÊÂ¼şIDÁĞ±í
x229024_g_EventList	= {}

x229024_g_Impact_Accept_Mission 	= 47	-- ½ÓÊÜÈÎÎñÊ±µÄÌØĞ§ID
x229024_g_PlayerSlow_LVL					= 10	-- ½ÓÊÜÈÎÎñµÄ×îµÍµÈ¼¶
...
x229024_g_Impact_Complete_Mission = 43				-- ÈÎÎñÍê³ÉÊ±µÄ¼ªÏéÈçÒâÌØĞ§
...
x229024_g_scenePosInfoList = { {sceneId=7,  scenename="Kiªm Các", PosName="Kiªm Môn Ği®p Thúy", PosX=130, PosY=140,  PosR=10, Area=711 },
															 ...
															 {sceneId=5,  scenename="Kính H°", PosName="Ng÷c Ğái Lâm Phong", PosX=35 , PosY=265,  PosR=10, Area=511 },
															 {sceneId=4,  scenename="Thái H°", PosName="Vû TÕ Ca Bàn", PosX=155, PosY=255,  PosR=10, Area=411 },
															 ...
															 {sceneId=25, scenename="Thß½ng S½n", PosName="Tñ Thüy Lßu Niên", PosX=260, PosY=110,  PosR=10, Area=2512},															 
...
function x229024_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if LuaFnGetName( sceneId, targetId ) ~= x229024_g_Name then
		x229024_NotifyTip( sceneId, selfId, "Tiªp nh§n nhi®m vø th¤t bÕi" )					
		return 0
	end
	...
	if key == x229024_g_QianXunMission_IDX then
		BeginEvent(sceneId)
			...
			AddNumText( sceneId, x229024_g_ScriptId, "Nh§n lînh nhi®m vø Phu Thê", 6, x229024_g_FuQiMission_IDX )		
			AddNumText( sceneId, x229024_g_ScriptId, "Nh§n lînh nhi®m vø Kªt Bái", 6, x229024_g_JieBaiMission_IDX )		
			AddNumText( sceneId, x229024_g_ScriptId, "Nh§n lînh nhi®m vø Sß Ğ°", 6, x229024_g_ShiTuMission_IDX )		
		EndEvent(sceneId)
	...
	elseif key == x229024_g_FuQiMission_IDX or key == x229024_g_JieBaiMission_IDX or key == x229024_g_ShiTuMission_IDX then
		...
		if IsHaveMission( sceneId, selfId, x229024_g_MissionId ) > 0 then
			...
			SetMissionData( sceneId, selfId, MD_QIANXUN_SELECT_MISSIONTYPE, key )			-- ÉèÖÃ¶Ó³¤ËùÑ¡µÄÈÎÎñÀàĞÍ
...
function x229024_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText( sceneId, x229024_g_ScriptId, "V« Thiên T¥m", 11, x229024_g_MissionInfo_IDX )
...
function x229024_OnAccept( sceneId, selfId, targetId, scriptId )
	...
	local	nMisType = GetMissionData( sceneId, selfId, MD_QIANXUN_SELECT_MISSIONTYPE )	--Íæ¼ÒËùÑ¡µÄÈÎÎñÀàĞÍ
	...
	if canAdd > 0 then						
		...
		if bAdd >= 1 then				
			...
			BeginEvent(sceneId)
				...
				local strText = format("Thiên T¥m nhi®m vø có 1 vòng: ngß½i chï có th¬ ğªn %s tÕi #{_INFOAIM%d,%d,%d,%s} m¾i có th¬ sØ døng t¥m lµ chi tâm", ScintInfo.scenename, ScintInfo.PosX, ScintInfo.PosY, ScintInfo.sceneId, ScintInfo.scenename)						
				AddText(sceneId,strText)
				AddText(sceneId,"#e00f000Nh¡c nhö: #e000000sØ døng #gfff0f0 phím chuµt phäi #g000000 nh¤n vào #gfff0f0T¥m Lµ Chi Tâm #g000000trong ô nhi®m vø, s¨ hi¬n th¸ ğ¸a ği¬m nhi®m vø c¥n tìm.")											
			EndEvent( )					
			DispatchEventList(sceneId, selfId, targetId)
...
function x229024_CheckSubmit( sceneId, selfId )
	...
	--µÃµ½ÈÎÎñµÄĞòÁĞºÅ
	local misIndex = GetMissionIndexByID(sceneId,selfId,x229024_g_MissionId)	
	if GetMissionParam(sceneId, selfId, misIndex, 0) > 0 then	--²ì¿´ÈÎÎñÊÇ·ñÍê³É		
		return 1
	end
...
function x229024_CheckMission_FuQi( sceneId, selfId )
  ...
  local curDayTime = nYear*10000+(nMonth+1)*100+nDay
	...
	--Èç¹ûÍæ¼ÒÒÑ¾­½ÓÁËÈÎÎñ
	if IsHaveMission( sceneId, selfId, x229024_g_MissionId ) > 0 then
		local nMisType 	= GetMissionData( sceneId, selfId, MD_QIANXUN_SELECT_MISSIONTYPE )				-- Íæ¼ÒÉíÉÏµÄÈÎÎñÀàĞÍ
		...
		nMisDaytime = GetMissionData( sceneId, selfId, MD_QIANXUN_FUQI_DAYTIME )			-- »ñµÃÉÏ´Î½ÓÊÜ·òÆŞ¹ØÏµÈÎÎñµÄÊ±¼ä
...
function x229024_CheckMission_JieBai( sceneId, selfId )
  ...
  local curDayTime = nYear*10000+(nMonth+1)*100+nDay
	...
	if IsHaveMission( sceneId, selfId, x229024_g_MissionId ) > 0 then	
		...
		nMisDaytime = GetMissionData( sceneId, selfId, MD_QIANXUN_JIEBAI_DAYTIME )		-- »ñµÃÉÏ´Î½ÓÊÜ½á°İ¹ØÏµÈÎÎñµÄÊ±¼ä
...
function x229024_CheckMission_ShiTu( sceneId, selfId )
  ...
  local curDayTime = nYear*10000+(nMonth+1)*100+nDay
	...
	if IsHaveMission( sceneId, selfId, x229024_g_MissionId ) > 0 then
		...
		nMisDaytime = GetMissionData( sceneId, selfId, MD_QIANXUN_SHITU_DAYTIME )			-- »ñµÃÉÏ´Î½ÓÊÜÊ¦Í½¹ØÏµÈÎÎñµÄÊ±¼ä
...
function x229024_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	--ÅĞ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
	if LuaFnGetName( sceneId, targetId ) ~= x229024_g_Name then
		x229024_NotifyTip( sceneId, selfId, "Giao nµp nhi®m vø th¤t bÕi" )					
		return 0
	end
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x229024_OnKillObject( sceneId, selfId, objdataId ,objId)--²ÎÊıÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎï
end
...
function x229024_OnEnterArea( sceneId, selfId, zoneId )
	...
	CreateSpecialObjByDataIndex(sceneId, selfId, x229024_g_Impact_Complete_Mission, x229024_g_scenePosInfoList[index].PosX+nposX, x229024_g_scenePosInfoList[index].PosY+nposY, 0)			-- ½øÈëÌ½Ë÷ÇøÓòºó²¥·ÅµÄÌØĞ§
end
...
function x229024_GetItemDetailInfo(itemId)
	...
	if itemId == -1 then
		local strText = format("%sV§t ph¦m tÕi 'EquipBase.txt' không tìm th¤y!!", itemName)		
	end
...
--*******************************************
--»òÕßÊÇ 2009Äê8ÔÂ24ÈÕ0µãÖÁ2009Äê8ÔÂ30ÈÕ24µã
--Á¬ĞøÒ»ÖÜ¿ªÆôÇ§Ñ°£¬czf£¬2009.08.07
--*******************************************
function x229024_CheckSpecialTime()

	local curDate = GetTime2Day( )
	
	--Èç¹ûÊÇÊÇ 2009Äê8ÔÂ24ÈÕ0µãÖÁ2009Äê8ÔÂ30ÈÕ24µãÁ¬ĞøÒ»ÖÜ£¬Ôò¿ªÆô
...
function x229024_OnUseItem( sceneId, selfId, bagIndex )
  ...
  if nRoundNum >= x229024_g_MaxRound then
  	local missitemid = GetItemTableIndexByIndex( sceneId, selfId, bagIndex )
		...
		-- 	LuaFnAuditPlayerBehavior(sceneId, selfId, "¿ªÆôÇ§Ñ°±¦Ïä");
		-- end
		...
		x229024_NotifyTip( sceneId, selfId, "Hoàn thành tình hu¯ng: ğã thåm dò "..nRoundNum.."/"..x229024_g_MaxRound )
		...
		str = format("Thiên T¥m nhi®m vø vòng thÑ %d: ği #G%s#W thåm dò #G%s#{_INFOAIM%d,%d,%d,%s}#W, v×a xem cänh ğ©p giang h° ği. Sau khi ğªn xin m¶i sØ døng #YT¥m lµ chi tâm#W.", nRoundNum, ScintInfo.scenename, ScintInfo.PosName, ScintInfo.PosX, ScintInfo.PosY, ScintInfo.sceneId, ScintInfo.scenename)	
