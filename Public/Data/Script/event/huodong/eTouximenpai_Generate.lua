...
--ºóË¢³ötoÕ ðµ ¹ÖÎïtoÕ ðµ ÊýÁ¿
x808015_g_RefreshMonsterNum = 5

--15 phútºóË¢³ötoÕ ðµ ¹ÖÎïtoÕ ðµ Î»ÖÃ
...
x808015_g_scenePosInfoList = {
	{sceneId=9, sceneLeader="Huy«n T×", sceneName="Thiªu Lâm", minL=30, maxL=45,  posList={
	...
	{sceneId=10, sceneLeader="T¯ng T×", sceneName="Cái Bang", minL=46, maxL=60,  posList={
	...
	{sceneId=11, sceneLeader="Lâm Thª Trß¶ng", sceneName="Minh Giáo", minL=61, maxL=75,  posList={
	...
	{sceneId=12, sceneLeader="Trß½ng Huy«n T¯", sceneName="Võ Ðang", minL=76, maxL=90,  posList={
	...
	{sceneId=13, sceneLeader="Bän Nhân", sceneName="Thiên Long", minL=91, maxL=100,posList={
	...
	{sceneId=14, sceneLeader="Tô Tinh Hà", sceneName="Tiêu Dao", minL=100, maxL=105, posList={
	...
	{sceneId=15, sceneLeader="MÕnh Thanh Thanh", sceneName="Nga My", minL=30, maxL=40,	posList={
	...
	{sceneId=16, sceneLeader="Ðinh Xuân Thu", sceneName="Tinh Túc", minL=41, maxL=55,posList={
	...
	{sceneId=17, sceneLeader="Mai Kiªm", sceneName="Thiên S½n", minL=56, maxL=70,posList={
				...
				if v.sceneId == sceneId then
				    ...
				    local j=1
						while j<=30 do
							...
							local index = j							
							--local strText = format( "µ±Ç°sceneid: %d, %d-%d,%d", sceneId, v.sceneId, v.posList[index].x, v.posList[index].z)
							--PrintStr(strText)
						...
						local NotifyMsg = "@*;SrvMsg;SCA:#P"..v.sceneLeader.."#P: tÕi #Y"..v.sceneName.." #Pðµt nhiên b¸ mµt t¯p #GGiang h° ti¬u tØ #Pðánh lén, ð® tØ môn hÕ các n½i mau v« môn phái quét sÕch kë ð¸ch."
...
function x808015_OnTimer( sceneId, actId, uTime )
		...
		if TickTime == 1 then
			...
			-- Ôö¼Ó5cái¹Ö,Î»ÖÃTÕi 30cáiÖÐËæ»ú
			for i, v in x808015_g_scenePosInfoList do
				if v.sceneId == sceneId then
					local j = 1
					while j<=x808015_g_RefreshMonsterNum do
