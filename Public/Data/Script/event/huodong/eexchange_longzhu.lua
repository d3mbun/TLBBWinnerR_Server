...
function x808058_AwardItem( sceneId, selfId, targetId, type )
	...
	BeginAddItem(sceneId)
		...
		elseif type == 3 then
			AddItem( sceneId, 30309826, 1 )
		elseif type == 4 then
			AddItem( sceneId, 30309036, 1 )
		end
...
function x808058_PlayerPickUpLongZhu( sceneId, selfId, bagidx )
	...
	local strNotice = format( "#{_INFOUSR%s}#P trong lúc ði dÕo ðµt nhiên phát hi®n trong bøi cö mµt viên hình tròn hÕt châu lóe hào quang, lát sau m¾i phát hi®n ðúng là #{_INFOMSG%s}.", PlayerName, szItemTransfer )
...
function x808058_OnDefaultEvent( sceneId, selfId, targetId )
    ...
    if 1 == TextNum then
		...
		BeginEvent(sceneId)
			AddNumText(sceneId, x808058_g_ScriptId, "Ta mu¯n ð±i l¤y Long Bài c¤p 65", 6, 3 )
			AddNumText(sceneId, x808058_g_ScriptId, "Ta mu¯n ð±i l¤y Long Bài c¤p 75", 6, 4 )
			AddNumText(sceneId, x808058_g_ScriptId, "Ta mu¯n ð±i l¤y Bång Long c¤p 85", 6, 5 )
			AddNumText(sceneId, x808058_g_ScriptId, "Ta mu¯n ð±i l¤y Giao Long c¤p 95", 6, 6 )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	...
	elseif 5 == TextNum then
		x808058_AwardItem( sceneId, selfId, targetId, 3 )
	elseif 6 == TextNum then
		x808058_AwardItem( sceneId, selfId, targetId, 4 )
    end
