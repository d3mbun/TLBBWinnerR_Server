...
function x125020_OnEnumerate( sceneId, selfId, targetId )	
   ...
   AddNumText( sceneId, x125020_g_ScriptId, "Gi¾i thi®u cÕnh kÛ trß¶ng", 11, 2 )         
end
...
function x125020_OnDefaultEvent( sceneId, selfId, targetId )
    ...
    BeginEvent(sceneId)
      AddText(sceneId,"#BGi¾i thi®u CÕnh KÛ Trß¶ng");
  ...
  if LuaFnGetDRideFlag(sceneId, selfId) ~= 0  then
    BeginEvent(sceneId)
     ...
     AddText(sceneId,"  TrÕng thái cùng cßÞi, không th¬ vào cÕnh kÛ trß¶ng");
    EndEvent(sceneId)
    DispatchEventList(sceneId,selfId,targetId)
    return 0
  end
  ...
  if GetLevel(sceneId, selfId) < 35  then
    BeginEvent(sceneId)
      AddText(sceneId,"#BCÕnh kÛ trß¶ng");
      AddText(sceneId,"  CÕnh kÛ trß¶ng là n½i tr± tài thñc lñc. Các hÕ chßa ðü thñc lñc ðâu ! Hãy luy®n t§p thêm ðªn ðÆng c¤p 35 r°i hãy ðªn ðây!");
    EndEvent(sceneId)
    DispatchEventList(sceneId,selfId,targetId)
    return 0
  end
  ...
  if LuaFnHasTeam( sceneId, selfId ) == 0  then
    BeginEvent(sceneId)
      AddText(sceneId,"#BCÕnh kÛ trß¶ng");
      AddText(sceneId,"    C¥n tham gia mµt t± ðµi m¾i có th¬ tiªn vào CÕnh KÛ Trß¶ng");
    EndEvent(sceneId)
    DispatchEventList(sceneId,selfId,targetId)
    return 0
	end
	...
	if GetItemCount(sceneId, selfId, x125020_g_Yinpiao)>=1  then
		BeginEvent( sceneId )
			AddText( sceneId, "  Các hÕ ðang trong trÕng thái v§n chuy¬n. Ð¸nh tr¯n nþ hä ?" )
		EndEvent( sceneId )
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
	...
	if IsHaveMission(sceneId,selfId, x125020_g_CaoyunMisId) > 0 then
		BeginEvent( sceneId )
			AddText( sceneId, "  Các hÕ ðang thñc hi®n nhi®m vø v§n chuy¬n. Không th¬ bß¾c vào Phong thi«n ðài." )
		EndEvent( sceneId )
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
...
function x125020_OnSceneTimer(sceneId)
	...
	if nCurTime==12 or nCurTime==20 then
		 -- nCurTime==12 or nCurTime==14 or nCurTime==16 or
		 -- nCurTime==18 or nCurTime==20 or nCurTime==22)   then
		--AddGlobalCountNews(sceneId,"x125020_g_Step: "..x125020_g_Step.."    "..(nCurTime-x125020_g_PreCreateBoxTime).."  vs  "..(x125020_g_PreCreateBoxTime-nCurTime ))	 
		if (nCurTime-x125020_g_PreCreateBoxTime >= 1 or x125020_g_PreCreateBoxTime-nCurTime>=1) and
			 x125020_g_Step == 0  and 
			 nMinute ==4    then	
			-- ÏÈ·¢mµt cáiÌáÊ¾¹«¸æ,mµt  phútºóÔÙË¢³ö±¦Ïä
...
function x125020_PreBroad(sceneId)
	local str = "#YVu CØu Liên#P hét l¾n: HÞi các anh hùng thiên hÕ, Sau mµt phút thì Bäo sß½ng chÑa vô s¯ bäo v§t và ði¬m kinh nghi®m ðã ðßþc ð£t tÕi Phong Thi«n Ðài ! Anh hùng nào mu¯n làm Võ Lâm Minh Chü, hãy mau mau ðªn #G LÕc Dß½ng(155,107) Tô Châu(186,129) ÐÕi Lý(177,133)"
...
function x125020_CreateBigBox(sceneId)
	...
	local str = "#YVu CØu Liên#P hét l¾n: HÞi các anh hùng thiên hÕ, Bäo sß½ng chÑa vô s¯ bäo v§t và ði¬m kinh nghi®m ðã ðßþc ð£t tÕi Phong Thi«n Ðài ! Anh hùng nào mu¯n làm Võ Lâm Minh Chü, hãy mau mau ðªn #G LÕc Dß½ng(155,107) Tô Châu(186,129) ÐÕi Lý(177,133)"
	BroadMsgByChatPipe(sceneId, 0, str, 4)
...
function x125020_RandomBroad(sceneId)
	if random(100) == 1  then
		...
		local str
		if rand == 1  then
			str = "#G[Phong thi«n ðài]#YVu CØu Liên#P : Hãy th¬ hi®n hªt khä nång cüa mình !!!"
		elseif rand == 2  then
			str = "#G[Phong thi«n ðài]#YVu CØu Liên#P : Chào m×ng các hÕ ðªn Phong Thi«n Ðài cüa #YThiên Long Bát Bµ "
		elseif rand == 3  then
			str = "#G[Phong thi«n ðài]#YVu CØu Liên#P : Chào m×ng các hÕ ðªn Phong Thi«n Ðài cüa #YThiên Long Bát Bµ #P hãy chiªn ð¤u ð¬ l¤y danh dñ cho chính mình."
		end
