...
10510005,10510006,10510012,10510013,10510017,10510023,10510026,
10510035,10510036,10510042,10510043,10510047,10510053,10510056,
10510065,10510066,10510072,10510073,10510077,10510083,10510086,
10511004,10511007,10511012,10511014,10511016,10511025,
10511034,10511037,10511042,10511044,10511046,10511055,
10511064,10511067,10511072,10511074,10511076,10511085,
10512003,10512012,10512015,10512017,10512023,10512026,10512033,
10512042,10512045,10512047,10512053,10512056,10512063,
10512072,10512075,10512077,10512083,10512086,10513004,10513014,
10513022,10513024,10513034,10513044,
10513052,10513054,10513064,10513074,10513082,
10513084,10514003,10514007,10514015,10514017,10514022,10514025,
10514033,10514037,10514045,10514047,10514052,10514055,
10514063,10514067,10514075,10514077,10514082,10514085,10515004,
...
10520024,10520027,10520032,10520035,10520036,10520043,
10520054,10520057,10520062,10520065,10520066,10520073,
10520084,10520087,10521002,10521004,10521007,10521014,
10521017,10521025,10521027,10521032,10521034,10521037,10521044,
10521047,10521055,10521057,10521062,10521064,10521067,10521074,
10521077,10521085,10521087,10522002,10522003,10522007,10522015,
10522023,10522026,10522032,10522033,10522037,10522045,
10522053,10522056,10522062,10522063,10522067,10522075,10522083,
...
10523077,10523084,10523086,10552005,10552015,10552016,10552024,
10552027,10552035,10552045,10552046,10552054,10552057,10552065,
10552075,10552076,10552084,10552087,10553007,10553016,
10553027,10553037,10553046,10553057,10553067,
10553076,10553087,

-- 10553078,10553068,10553018,10553038,10553008,10552068,10552008,
-- 10552038,10522018,10522048,10521088,10521058,10520088,10522078,
-- 10520078,10520048,10520018,10514028,10513018,10512038,10512068,
-- 10510008,10510038,10510068,10511018,10511028,10512008,10513048,
-- 10511048,10513038,10513068,10514088,10514058,10520028,10520058,
-- 10511078,10513088,10513028,10513058,10513078,10511058,10511088,
-- 10513008,10521028,

-- 10500009,10500019,10501009,10502009,
...
function x125022_OnActivateConditionCheck( sceneId, selfId, activatorId )
	...
	if bOk == 0  then
    BeginEvent(sceneId)
      AddText(sceneId,"Các hÕ không th¬ m· bäo sß½ng");
    EndEvent(sceneId)
    ...
    BeginEvent(sceneId)
      AddText(sceneId,"Không · t± ðµi ðúng, không th¬ m· bäo sß½ng");
    EndEvent(sceneId)
    DispatchMissionTips(sceneId,activatorId,selfId)
		bOk = 0
	end
	...
	if bOk == 1  then
		local str = "#G[Phong Thi«n Ðài]#W" .. GetName(sceneId, activatorId) .."#P ðang m· bäo sß½ng."
...
function x125022_IsCanOpenBox( sceneId,activatorId )
	...
	if nMinute == 4 then 
		BeginEvent(sceneId)
		  AddText(sceneId,"Hi®n tÕi không th¬ m· bäo sß½ng");
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,activatorId,selfId)	
		return 0
	end
...
function x125022_OnActivateEffectOnce( sceneId, selfId, activatorId )
	...
	for i=0, nCount-1  do
		...
		if GetName(sceneId, nObjId) == "Hoàng S¡c Bäo Sß½ng"  then
			bDelOk = 1
	...
	if bDelOk == 1  then
		...
		if nBoxId > -1  then
			...
			local str = format("#YVu CØu Liên#P hét l¾n : HÞi anh hùng thiên hÕ ! Ðã có #{_INFOUSR%s}#P ðoÕt ðßþc Võ Lâm Minh Chü Bäo Sß½ng ! M÷i ngß¶i vào lúc #Y12 gi¶ 00 phút và  20 gi¶ 00 phút #Phãy lÕi ðªn#G Phong Thi«n Ðài #Ptranh ðoÕt Võ Lâm Minh Chü !",GetName(sceneId,activatorId))
	...
	LuaFnAuditPlayerBehavior(sceneId, activatorId, "Khai m· minh chü bäo sß½ng");
...
function x125022_OnActivateCancel( sceneId, selfId, activatorId )
	local str = "#G[Phong Thi«n Ðài]#W" .. GetName(sceneId,activatorId) .. "#P m· bäo sß½ng th¤t bÕi."
	CallScriptFunction((200060), "Duibai",sceneId, "", "", str)
	return 0
end
