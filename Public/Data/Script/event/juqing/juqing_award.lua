...
function x200099_OnNewMission(sceneId, selfId)
	if GetMoney(sceneId,selfId) == GetTodayMonth() then
		local SpaceDC = LuaFnGetPropertyBagSpace(sceneId,selfId)
		local SpaceNL = LuaFnGetMaterialBagSpace(sceneId,selfId)
		if GetTodayDate() == SpaceNL + SpaceDC then
			local str = GetDayTime()
			BeginEvent(sceneId)
				for h = GetTodayWeek(), GetTodayYear() do
					AddText(sceneId,str)
				end
