	...
	if nStep1 == 1  then
		...
		elseif nStep2 == 3 then
			...
			if nStep2 == 19 then
				...
				CallScriptFunction((200060), "Duibai",sceneId, "Tiªt Mµ Hoa", "Tø Hi«n Trang", "Ki«u Phong, ngß½i sao thª r°i. Còn cÑu con a ğ¥u này à ?" )
				LuaFnSetCopySceneData_Param(sceneId, 13, nStep2+1)
				LuaFnSetCopySceneData_Param(sceneId, 12, nNowTime)
			end
			...
			if nStep2 == 23 then
				...
				CallScriptFunction((200060), "Duibai",sceneId, "Ki«u Phong", "Tø Hi«n Trang", "Ki«u m² tñ mình kªt li­u" )
				LuaFnSetCopySceneData_Param(sceneId, 13, nStep2+1)
				LuaFnSetCopySceneData_Param(sceneId, 12, nNowTime)
			end
			...
			if nStep2 == 25 then
				if nNowTime - nPreTime > 3  then
					LuaFnSetCopySceneData_Param(sceneId, 13, nStep2+1)
					LuaFnSetCopySceneData_Param(sceneId, 12, nNowTime)
					CallScriptFunction((200060), "Duibai",sceneId, "Ki«u Phong", "Tø Hi«n Trang", "$N, Ta giao A Châu cho ngß½i ğ¤y! Ği tìm Tiªt Th¥n Y cÑu m®nh cüa cô ta, ği mau!" )
				end
			end
			...
			if nStep2 == 27 then
				...
				for i=0, nHumanNum-1  do
					...
					-- ÌáÊ¾Íæ¼Ò¾çÇéÍê³É
					BeginEvent(sceneId)
						AddText(sceneId, "Ğã bäo hµ Ki«u Phong thoát nguy: 1/1")
					EndEvent(sceneId)
