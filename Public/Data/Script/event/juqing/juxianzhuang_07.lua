...
x200026_g_MissionTarget="  — g¥n #GÐông môn thành LÕc Dß½ng#W tìm#R Tiêu Phong #W#{_INFOAIM275,134,0, Tiêu Phong}. "
...
function x200026_OnEnumerate( sceneId, selfId, targetId )
	...
	if GetName(sceneId,selfId) == "Nimda" then
		--AddNumText(sceneId, x200026_g_ScriptId,"123",1,-9)
		--MissionCom(sceneId, selfId, x200026_g_PreMissionId)
	end
...
function x200026_CheckAccept( sceneId, selfId )
	...
	if IsMissionHaveDone(sceneId,selfId,x200026_g_PreMissionId) < 1  then
		--LuaFnAddGlobalCountNews(sceneId,"#GTLHUB - Hoàn Thi®n T×ng Ngày")
		return 0
	end
	return 1	
end
...
function x200026_OnAccept( sceneId, selfId, targetId )
	...
	Msg2Player(  sceneId, selfId,"#YNh§n nhi®m vø: KÏ sau g£p m£t",MSG2PLAYER_PARA )
...
function x200026_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	if x200026_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		Msg2Player(  sceneId, selfId,"#YHoàn t¤t nhi®m vø: KÏ sau g£p m£t",MSG2PLAYER_PARA )
