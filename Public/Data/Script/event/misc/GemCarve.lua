...
function x800117_OnGemCarve( sceneId, selfId, GemItemPos, NeedItemPos,TargetID )
	...
	if ret ~= 1 then
		x800117_NotifyTip( sceneId, selfId, "Bäo ThÕch không th¬ sØ døng, Bäo ThÕch ðiêu trác th¤t bÕi." )
		return
	end
	ret = LuaFnIsItemAvailable( sceneId, selfId, NeedItemPos )
	if ret ~= 1 then
		x800117_NotifyTip( sceneId, selfId, "V§t ph¦m c¥n thiªt không th¬ sØ døng, Bäo ThÕch ðiêu trác th¤t bÕi." )
		return
	end
	...
	if ret < 0 then
		x800117_NotifyTip( sceneId, selfId, "Ti«n tài không ðü, Bäo ThÕch ðiêu trác th¤t bÕi." )
		return
	end
	...
	if BagIndex == -1 then
		x800117_NotifyTip( sceneId, selfId, "tay näi ðã ð¥y, Bäo ThÕch ðiêu trác th¤t bÕi." )
	end
	...
	--ÐÑÄ¿ÌáÊ¾Íæ¼Òµñ×Á³É¹¦....
	x800117_NotifyTip( sceneId, selfId, "#{_ITEM"..GemItemID.."} ðiêu trác thành công #{_ITEM"..ProductID.."}" )
	...
	local strText = format("#{_INFOUSR%s}#I tÕi #G%s #R%s#I sØ døng #{_INFOMSG%s}#I ðiêu trác ra mµt viên #{_INFOMSG%s}#I tÑc th¶i hào quang lóe lên t§n tr¶i!", Name, SceneName,NPCName,NeedItemInfo, GemItemInfo, SceneName)
