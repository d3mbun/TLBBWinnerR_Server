...
function x800112_CheckCost( sceneId, selfId, targetId ,nValue )
	...
	if not gb_value or gb_value < nValue * 2000 then
		x800112_NotifyFail( sceneId, selfId, "Giäi tr× sát khí các hÕ c¥n " .. nValue * 2000 .. " ði¬m thi®n ác, hi®n tÕi ði¬m thi®n ác cüa các hÕ không ðü." ,targetId)
...
--		x800112_NotifyFailTips(sceneId, selfId, "Ïû³ýÄúµÄÉ±ÆøÐèÒª" .. nValue * 2000 .. "µãÉÆ¶ñÖµ£¬Ä¿Ç°ÄúÉíÉÏµÄÉÆ¶ñµãÊý²»×ã¡£");
	...
	if not money or money < nValue * 50000 then
		x800112_NotifyFail( sceneId, selfId, "Ð¬ tr× sát khí các hÕ c¥n #{_MONEY" .. nValue * 50000 .. "}, ngân lßþng cüa các hÕ không ðü.", targetId )
