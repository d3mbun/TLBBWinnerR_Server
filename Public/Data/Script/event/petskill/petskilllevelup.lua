...
function x311112_PetSkillLevelup( sceneId, selfId, petHid, petLid, skillindex, ItemIndexInBag )
	-- if 1 == 1 then
		-- x311112_NotifyFailTips(sceneId, selfId, "ChÑc nång tÕm khóa")
		-- return
	-- end
	...
	if checkAvailable and checkAvailable == 1 then
		...
		if IsBroadCast == 1 then
			...
			local message = format("#H#{_INFOUSR%s} cho #{_INFOMSG%s} nâng c¤p #G%s#P.",LuaFnGetName(sceneId, selfId),szPetTrans,GetSkillName(SkillLevelUpID))--[tx43999]
