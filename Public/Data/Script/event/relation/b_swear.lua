...
x806001_g_msg_swear["mon"]	= " Tiªn hành nghi thÑc kªt bái c¥n t¯n #{_EXCHG%d}, ngß½i xác ð¸nh có c¥n kªt bái không? "
x806001_g_msg_swear["nom"]	= " Ngân lßþng cüa các hÕ không ðü #{_EXCHG%d}"
...
x806001_g_msg_swear["wait"]	= " Ch¶ ðþi nhæng ngß¶i khác h°i phøc."
...
function x806001_CheckAccept( sceneId, selfId, targetId )
	...
	while i < TeamSizeSameScene do
		...
		if IsInDist( sceneId, theID, targetId, 6 ) ~= 1 then
			...
			x806001_MessageBox( sceneId, selfId, targetId, " trong nhóm cüa các hÕ #R" .. theName .. "#W cách ta quá xa, nên ta không th¬ kªt bái cho các ngß½i" )
		...
		if theSwear > 0 then --¶ÓÔ±ÓÐ½á°Ý¹ØÏµ
			if BrotherNum > 0 then --¶Ó³¤ÓÐ½á°Ý¹ØÏµ
				if LuaFnIsBrother(sceneId, selfId, theID) == 0 then
					local	theName	= LuaFnGetName( sceneId, theID )
					x806001_MessageBox( sceneId, selfId, targetId, " trong nhóm cüa các hÕ " .. theName .. " Ðã là huynh ð® kªt bái cüa ngß¶i khác, ta không tài nào chü trì nghi l­ kªt bái cho các ngß½i ðßþc" )
					return 0
				end
			else
				local	theName	= LuaFnGetName( sceneId, theID )
...
function x806001_OnSubmit( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		...
		AddText( sceneId, msg )
		AddNumText( sceneId, x806001_g_ScriptId, "Duy®t", 6, x806001_g_key["allow"] )
		AddNumText( sceneId, x806001_g_ScriptId, "Hüy", 8, x806001_g_key["unall"] )
	EndEvent( sceneId )
	DispatchEventList( sceneId, selfId, targetId )
end
...
function x806001_QuitSwear( sceneId, selfId, targetId )
	...
	if TeamLeaderID ~= nil then
		...
		local	msg = LuaFnGetName( sceneId, selfId ) .. "Bö vi®c kªt bái v¾i huynh"
...
function x806001_WaitSwear( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x806001_g_ScriptId, "Ta không mu¯n kªt bái", 8, x806001_g_key["uncon"] )
	EndEvent( sceneId )
	DispatchEventList( sceneId, selfId, targetId )
end
