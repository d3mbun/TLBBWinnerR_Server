...
function x806020_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if x806020_IsValidateTime_Change() ==0 then
		x806020_NotifyMsg( sceneId, selfId, targetId,  "TÕm th¶i không th¬ ð±i ph¥n thß·ng haot5 ðµng" )
	end
	...
	if(GetNumText() == 1) then
		BeginEvent(sceneId)
			...
			AddNumText( sceneId, x806020_g_ScriptId, "Ð±i l¤y ÐoÕt Bäo Sß Ð° Luy®n BUFF", 6, 5 )
		EndEvent( sceneId )
	...
	elseif (GetNumText() == 6) or (GetNumText() == 7) then
		...
		elseif GetNumText() == 7 then
			levelReq = 45
			strTip = "Ð° ð® thång ðªn c¤p 45 m¾i có th¬ lînh nh§n ph¥n thß·ng!"
			strMsg = "Ð° ð® c¥n thång c¤p 45 m¾i có th¬ lînh nh§n ph¥n thß·ng!"
		...
		for i=1, getn(x806020_g_Need_Item_List) do
			if LuaFnGetAvailableItemCount(sceneId, selfId, x806020_g_Need_Item_List[i]) > 0 then				
				...
				if LuaFnDelAvailableItem(sceneId, selfId, x806020_g_Need_Item_List[i], 1) == 1 then
					...
					x806020_NotifyMsg( sceneId, selfId, targetId,  "Trao ð±i thành công." )
