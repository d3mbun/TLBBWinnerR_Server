...
function x806003_OpenMarryLevelUI(sceneId, selfId, targetId)
	BeginEvent(sceneId);
		...
		AddText(sceneId, "#{yuelao_jiehun}#rhôn l­ thông thß¶ng, c¥n tiêu t¯n #{_EXCHG"..moneyLevel0.."}, hôn l­ quy mô b§c trung, c¥n tiêu t¯n #{_EXCHG"..moneyLevel1.."}, hôn l­ quy mô hào hoa, c¥n tiêu t¯n #{_EXCHG"..moneyLevel2.."}.");
...
function x806003_SelfCheckSelectMarryLevel(sceneId, selfId, targetId, marryLevel)
	...
	BeginEvent(sceneId);
		if marryLevel == 2 then
			AddText(sceneId, "Các hÕ ðã ch÷n r°i "..x806003_GetMarryLevelName(marryLevel)..", ð°ng th¶i các hÕ s¨ phäi trä #Y"..GetItemName(sceneId, x806003_g_ticketItemId).."#W ho£c #{_EXCHG"..x806003_CalcNeedMoney(marryLevel).."}, #r#rCác ngß½i ðã quyªt ð¸nh l§p gia th¤t chßa, và cuµc s¯ng tiªp t¾i s¨ b¤t di b¤t d¸ch?");
		else
			AddText(sceneId, "Các hÕ ðã ch÷n r°i "..x806003_GetMarryLevelName(marryLevel)..", ð°ng th¶i các hÕ s¨ phäi trä #Y"..GetItemName(sceneId, x806003_g_ticketItemId).."#W ho£c #Y"..GetItemName(sceneId, x806003_g_ticketItemId2).."#W ho£c #{_EXCHG"..x806003_CalcNeedMoney(marryLevel).."}, #r#rCác ngß½i ðã quyªt ð¸nh l§p gia th¤t chßa, và cuµc s¯ng tiªp t¾i s¨ b¤t di b¤t d¸ch?");
		end
		...
		AddNumText(sceneId, x806003_g_ScriptId, "Ð°ng ý", 6, tempAcceptEventID);
		AddNumText(sceneId, x806003_g_ScriptId, "Hüy", 8, x806003_g_eventId_selfCancelLevel);
	EndEvent(sceneId);
	DispatchEventList(sceneId, selfId, targetId);
...
function x806003_TargetCheckSelectMarryLevel(sceneId, selfId, targetId, marryLevel)
	...
	if marryTargetId ~= -1 then
		...
		BeginEvent(sceneId);
			if marryLevel == 2 then
				AddText(sceneId, "Ð¯i phß½ng ðã ch÷n xong"..x806003_GetMarryLevelName(marryLevel)..", ð°ng th¶i ð¯i phß½ng s¨ phäi trä#Y"..GetItemName(sceneId, x806003_g_ticketItemId).."#W ho£c #{_EXCHG"..x806003_CalcNeedMoney(marryLevel).."}, #r#rCác ngß½i ðã quyªt ð¸nh l§p gia th¤t chßa, và cuµc s¯ng tiªp t¾i s¨ b¤t di b¤t d¸ch?");
			else
				AddText(sceneId, "Ð¯i phß½ng ðã ch÷n xong"..x806003_GetMarryLevelName(marryLevel)..", ð°ng th¶i ð¯i phß½ng s¨ phäi trä#Y"..GetItemName(sceneId, x806003_g_ticketItemId).."#W ho£c #Y"..GetItemName(sceneId, x806003_g_ticketItemId2).."#W ho£c #{_EXCHG"..x806003_CalcNeedMoney(marryLevel).."}, #r#rCác ngß½i ðã quyªt ð¸nh l§p gia th¤t chßa, và cuµc s¯ng tiªp t¾i s¨ b¤t di b¤t d¸ch?");
			end
			...
			AddNumText(sceneId, x806003_g_ScriptId, "Ð°ng ý", 6, tempAcceptEventID);
			AddNumText(sceneId, x806003_g_ScriptId, "Hüy", 8, x806003_g_eventId_targetCancelLevel);
		EndEvent(sceneId);
...
function x806003_OnTargetCancelSelectMarryLevel(sceneId, selfId, targetId, marryLevel)
	...
	if marryTargetId ~= -1 then
		...
		x806003_MessageBox(sceneId, marryTargetId, targetId, "Ð¯i phß½ng ðã t× ch¯i.");
...
function x806003_CheckMarry(sceneId, selfId, targetId, marryLevel, isCheckOpenMarryLevelUI)
	...
	if ticketItemPos and ticketItemPos > -1 then
		...
		if not checkRet or checkRet ~= 1 then
			x806003_MessageBox(sceneId, selfId, targetId, "Cüa các hÕ #Y"..GetItemName(sceneId, selectTicketItemId).."#Wb¸ khóa ð¸nh r°i, cho nên bây gi¶ không th¬ giao cho ta.");
			x806003_MessageBox(sceneId, marryTargetId, targetId, "Ð¯i phß½ng #Y"..GetItemName(sceneId, selectTicketItemId).."#Wb¸ khóa ð¸nh r°i, cho nên bây gi¶ không th¬ giao cho ta.");
			return 0, -1, -1;
		end
	else
	--hzp 2008-12-9
...
function x806003_DoMarry(sceneId, selfId, targetId, marryLevel, maleId, femaleId)
	...
	Msg2Player(sceneId, maleId, "Các hÕ ðÕt thêm danh xßng  '"..femaleName.." Phu Quân'.", MSG2PLAYER_PARA);

	--ÁÄÌì´°¿Ú¸øÅ®·½ÌáÊ¾
	--Msg2Player(sceneId, femaleId, "¹§Ï²ÄãºÍ"..maleName.."½áÎª·òÆÞ£¡", MSG2PLAYER_PARA);
	Msg2Player(sceneId, femaleId, "Các hÕ ðÕt thêm danh xßng  '"..maleName.." Nß½ng TØ'.", MSG2PLAYER_PARA);
	...
	--ÊÀ½ç¹«¸æ
	local strChatMessage = "#b#cff99ffXin chúc m×ng #W#{_INFOUSR"..maleName.."}#b#cff99ff và #W#{_INFOUSR"..femaleName.."}#b#cff99ff hÖ kªt lß½ng duyên, nguy®n chúc cho hai ngß¶i ð¥u bÕc rång long, vînh kªt ð°ng tâm, hÕnh phúc su¯t ð¶i!";
