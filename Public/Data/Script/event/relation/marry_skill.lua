...
--Hình Änh B¤t Ly c¤p 1¶ÔÓ¦µÄ¼¼ÄÜID
x806016_g_xybl_SkillID = 269
--Ñ§Ï°Hình Änh B¤t Ly c¤p 1ÐèÒªµÄÎïÆ·ID
...
function x806016_OnDefaultEvent(sceneId, selfId, targetId)
	...
	if selectEventId == 0 then
		BeginEvent(sceneId);
			AddNumText(sceneId, x806016_g_ScriptId, "H÷c t§p Ý hþp tâm ð¥u", 12, 11);
			AddNumText(sceneId, x806016_g_ScriptId, "H÷c t§p Ð°ng khí liên chi", 12, 12);
			AddNumText(sceneId, x806016_g_ScriptId, "H÷c t§p Thiên trß¶ng ð¸a cØu", 12, 13);
			AddNumText(sceneId, x806016_g_ScriptId, "H÷c t§p Hình änh b¤t ly", 12, 14);
		EndEvent(sceneId);
		...
		if ret > 0 then
			...
			BeginEvent(sceneId);
				...
				if skill.money > 0 then
					AddText(sceneId, "  Nhà trai c¥n t¯n #{_EXCHG"..skill.money.."} h÷c t§p "..skill.name..".");
				end
				if nextLevel > 0 then
	...
	elseif selectEventId == 12 then
		...
		if ret > 0 then
			...
			BeginEvent(sceneId);
				...
				AddText(sceneId, "  H÷c t§p "..skill.name.." Nhà trai c¥n t¯n "..skill.exp.." kinh nghi®m cùng #{_EXCHG"..skill.money.."}.");
				AddText(sceneId, "  Ð°ng th¶i c¥n vþ ch°ng häo hæu ðµ ðÕt t¾i #G"..skill.firendPt.."#W, nhà trai c¤p b§c ðÕt t¾i "..skill.lvM.." c¤p, nhà gái c¤p b§c ðÕt t¾i "..skill.lvF.."c¤p.");
				if nextLevel > 0 then
	...
	elseif selectEventId == 13 then
		...
		if ret > 0 then
			...
			BeginEvent(sceneId);
				AddText(sceneId, "  Thiên trß¶ng ð¸a cØu có h°i sinh lÕi ph¯i ngçu cûng h°i phøc nh¤t ð¸nh tï l® huyªt khí, kÛ nång c¤p b§c càng cao, th¶i gian phøc h°i càng ít.");
				AddText(sceneId, "  H÷c t§p "..skill.name.." Nhà trai c¥n t¯n "..skill.exp.." kinh nghi®m cùng #{_EXCHG"..skill.money.."}.");
				AddText(sceneId, "  Ð°ng th¶i c¥n vþ ch°ng häo hæu ðµ ðÕt t¾i #G"..skill.firendPt.."#W, nhà trai c¤p b§c ðÕt t¾i "..skill.lvM.." c¤p, nhà gái c¤p b§c ðÕt t¾i "..skill.lvF.."c¤p.");
				if nextLevel > 0 then
	...
	elseif selectEventId == 14 then
		...
		if ret > 0 then
			...
			BeginEvent(sceneId);
				AddText(sceneId, "  Hình Änh B¤t Ly có th¬ trong nháy m¡t di chuy¬n ðªn v¸ trí vþ ch°ng, kÛ nång c¤p b§c càng cao, th¶i gian càng h°i phøc ít.");
				if skill.id == x806016_g_xybl_SkillID then
					AddText(sceneId, "  H÷c t§p "..skill.name.." nhà trai c¥n có #G#{_ITEM"..tostring(x806016_g_xybl_ItemID).."}#W.");
				else
					AddText(sceneId, "  H÷c t§p "..skill.name.." Nhà trai c¥n t¯n "..skill.exp.." kinh nghi®m cùng #{_EXCHG"..skill.money.."}.");
				end
				AddText(sceneId, "  Ð°ng th¶i c¥n vþ ch°ng häo hæu ðµ ðÕt t¾i #G"..skill.firendPt.."#W, nhà trai c¤p b§c ðÕt t¾i "..skill.lvM.." c¤p, nhà gái c¤p b§c ðÕt t¾i "..skill.lvF.."c¤p.");
...
function x806016_OnEnumerate(sceneId, selfId, targetId)
	...
	if isMarried and isMarried > 0 then
		AddNumText(sceneId, x806016_g_ScriptId, "KÛ nång h÷c làm phu thê", 12, 0);
	end
end
...
function x806016_CheckStudySkill( sceneId, selfId, targetId, SkillList )
	...
	--Ç°ÌáÌõ¼þ
	local szMsg = "Nªu mu¯n h÷c kÛ nång phu thê, m¶i 2 bên nhà trai và nhà gái cùng h÷p thành mµt nhóm r°i t¾i tìm ta"
	...
	szMsg = "Hai bên b¡t buµc phäi tr· thành phu thê m¾i có ðßþc kÛ nång phu thê."
...
function x806016_StudySkill(sceneId, selfId, targetId, SkillList)
	...
	szMsg = "Nhà trai c¤p b§c phäi ðÕt t¾i %d m¾i có th¬ h÷c ðßþc tiªp theo hÕng kÛ nång phu thê."
	...
	szMsg = "Nhà gái c¤p b§c phäi ðÕt t¾i %d m¾i có th¬ h÷c ðßþc tiªp theo hÕng kÛ nång phu thê."
	...
	if skill.id == x806016_g_xybl_SkillID then
		if HaveItemInBag(sceneId,maleId,x806016_g_xybl_ItemID)<0 then
			x806016_MessageBox(sceneId, selfId, targetId, "Nhà trai c¥n có #G#{_ITEM"..tostring(x806016_g_xybl_ItemID).."}#W tài nång h÷c t§p nên kÛ nång!");
			return
		end
		if LuaFnGetAvailableItemCount(sceneId,maleId,x806016_g_xybl_ItemID)<=0 then
			x806016_MessageBox(sceneId, selfId, targetId, "V§t ph¦m c¥n thiªt #G#{_ITEM"..tostring(x806016_g_xybl_ItemID).."}#W ðã b¸ khóa, thïnh giäi khóa xong h÷c lÕi!");
			return
		end
		--¿Û³ýÎïÆ·
		LuaFnDelAvailableItem(sceneId,maleId,x806016_g_xybl_ItemID,1)
	else
		szMsg = "Nhà trai kinh nghi®m phäi ðÕt t¾i %d m¾i có th¬ h÷c ðßþc tiªp theo hÕng kÛ nång phu thê."
		...
		szMsg = "Nhà trai c¥n mang theo #{_EXCHG%d} m¾i có th¬ h÷c t§p kÛ nång."
...
function x806016_MyAddSkill(sceneId, selfId, SkillList, nextLevel)
	...
	--Msg2Player( sceneId, selfId, "Các hÕ ðã h÷c ðßþc "..skill.name..".", MSG2PLAYER_PARA )
	
	--BeginEvent(sceneId)
		--AddText(sceneId, "Các hÕ ðã h÷c ðßþc "..skill.name..".");
	--EndEvent(sceneId)
...
--Ñ§Ï°Hình Änh B¤t Ly c¤p 1Ê±µÄÌáÊ¾ÐÅÏ¢
function x806016_SendSkillMsg_XYBL(sceneId,selfId,skname)
	if skname then
		local str = format("#IQuý vþ ch°ng ð°ng tâm hi®p lñc, bài tr× muôn vàn khó khån, · nhà trai hao phí #Y#{_ITEM"..tostring(x806016_g_xybl_ItemID).."}#I r¯t cøc h÷c xong kÛ nång phu thê#Y%s.",skname)
		BeginEvent(sceneId)
			AddText(sceneId, str);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
...
function x806016_SendSkillMsg(sceneId,selfId,exp,money,skname)
	if exp and money and skname then
		...
		local str = format("#IQuý vþ ch°ng ð°ng tâm hi®p lñc, bài tr× muôn vàn khó khån, · nhà trai hao phí  #Y%d kinh nghi®m cùng %s ti«n tài #I r¯t cøc h÷c xong kÛ nång phu thê #Y%s.",exp,moneyt,skname)
		BeginEvent(sceneId)
			AddText(sceneId, str);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	...
	if maleId and femaleId and skill then
		for _, tempId in x806016_g_MaxMarrySkill_T do
			if tempId == skill.id then
					...
					local str = format("#W%s#I cùng #W%s#I vþ ch°ng ð°ng tâm, r¯t cøc kh± tçn cam lai, h÷c xong #cff66cc%s#I, không h± là mçu cho các c£p phu thê.",uname,oname,skill.name)
