		...
		BeginEvent( sceneId )
			if LuaFnHaveMaster( sceneId, selfId ) == 0 then
				...
				if GetMissionFlag(sceneId, selfId, MF_ShiTu_ChuShi_Flag) == 1 then --±íÊ¾ÒÑ¾­³öÊ¦
					AddText( sceneId, "  Ngß½i ğã mu¯n xu¤t sß, không ğßşc có hành vi ph¦n sß." )
				else
					AddText( sceneId, "  Ngß½i vçn chßa bái sß!" )
				end
			else
				AddText( sceneId, str )
				AddNumText( sceneId, x806006_g_ScriptId, "Có", 6, 1 )
				AddNumText( sceneId, x806006_g_ScriptId, "Không", 6, 2 )	--add by xindefeng
			end
		EndEvent( sceneId )
...
function x806006_OnAccept( sceneId, selfId, targetId )
	...
	if nMoneyJB ~= 0 and nMoneyJZ ~= 0 then
		str	= format( "Bö quan h® sß-ğ° b¸ phÕt #{_EXCHG%d} ho£c ".."#{_MONEY%d}.", nMoneyJZ, nMoneyJB )
		Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
	end
	x806006_MsgBox( sceneId, selfId, targetId, "  Ngß½i ğã bö quan h® v¾i "..MasterName.." quan h® sß ğ° cüa" )

	--·¢ÆÕÍ¨ÓÊ¼ş¸øÊ¦¸µ
	LuaFnSendSystemMail( sceneId, MasterName, "Ğ® tØ cüa các hÕ " .. selfName .. "không c¥n sñ dÕy d² cüa các hÕ næa nên ğã thoát ly quan h® sß ğ°." )
