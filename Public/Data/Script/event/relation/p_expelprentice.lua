...
x806009_g_ExpelPrentice["Name"]	= "Khai tr◊ ra khˆi sﬂ mÙn"
...
function x806009_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == -1 then
		...
		for i=0, 7 do
			...
			if guid ~= -1 then
				...
				AddNumText( sceneId, x806009_g_ScriptId, "S® tr¯c xu§t "..PrenticeName.." ra khˆi sﬂ mÙn", 6, i )
			end
		end
	...
	if key >= 0 and key <=7 then
		...
		BeginEvent( sceneId )
			AddText( sceneId, str )
			AddNumText( sceneId, x806009_g_ScriptId, "CÛ", 6, (key+1)*100 )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
	end
...
function x806009_OnAccept( sceneId, selfId, targetId, nIndex )
	...
	if nMoneyJB ~= 0 and nMoneyJZ ~= 0 then
		str	= format( "Bˆ quan hÆ sﬂ-∞ b∏ ph’t #{_EXCHG%d} ho£c #{_MONEY%d}.", nMoneyJZ, nMoneyJB )
		Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
	end
	x806009_MsgBox( sceneId, selfId, targetId, "  NgﬂΩi „ bˆ quan hÆ væi "..PrenticeName.." quan hÆ sﬂ ∞ c¸a" )

	--∑¢∆’Õ®” º˛∏¯ÕΩµ‹
	LuaFnSendSystemMail( sceneId, PrenticeName, "Sﬂ ph¯ c¸a ngﬂΩi, " .. selfName .. ", khÙng ti™p t¯c d’y b‰o v‡ „ tho·t khˆi quan hÆ sﬂ ∞ væi ngﬂΩi." )
	--LuaFnSendNormalMail( sceneId, selfId, PrenticeName, "Sﬂ ph¯ c¸a ngﬂΩi, " .. selfName .. ", khÙng ti™p t¯c d’y b‰o v‡ „ tho·t khˆi quan hÆ sﬂ ∞ væi ngﬂΩi." )
	--∑¢ø…÷¥––” º˛∏¯ÕΩµ‹
