...
function x806015_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x806015_g_Update["wu"] then
		...
		local msg = "ÐÆng c¤p sß ðÑc cüa các hÕ ðßþc thång t¾i" .. tostring( LuaFnGetmasterLevel( sceneId, selfId ) .. " c¤p." )
...
function x806015_Upgrade( sceneId, selfId, targetId )
	...
	local text = "  ÐÆng c¤p sß ðÑc s¨ ðßþc thång c¤p t¾i " .. tostring( level + 1 ) .. " c¤p, s¨ tiêu hao tr¸ giá thi®n ác " .. tostring( exp ) .. " ði¬m."
	BeginEvent( sceneId )
...
--	x806015_MessageBox( sceneId, selfId, targetId, msg )
	...
	if gbvalue < exp then
		local msg = "  Tr¸ giá thi®n ác cüa các hÕ không ðü. Tr¸ giá thi®n ác cüa các hÕ hi®n là " .. tostring( gbvalue ) .. "."
		x806015_MessageBox( sceneId, selfId, targetId, msg )
		return 0
	end
	...
	local	str	= format( "#IXin chúc m×ng #W#{_INFOUSR%s} #Ithành công ðÕt ðßþc danh xßng #G%s#I, có th¬ thu nh§n nhæng ngß¶i ch½i khác làm ð° ð®.", nam, x806015_g_Title[level] )
