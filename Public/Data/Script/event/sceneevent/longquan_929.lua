...
--∞—ŒÃºæ÷Æµƒ‚„ª⁄–≈ÀÕ∏¯Ω…ΩÀ¬µƒPh·p NiÆm°£
...
x212119_g_MisItemId = 40004418 --ŒÃºæ÷Æµƒ‚„ª⁄–≈
...
function x212119_OnAccept( sceneId, selfId, targetId )
	...
	-- ºÏ≤ÈÕÊº“µƒ–ƒ∑® «≤ª «¥ÔµΩ¡À“™«Û£®«∞¡˘÷÷–ƒ∑®µ»º∂¥Û”⁄µ»”⁄15º∂£©
	if CallScriptFunction( 212120, "CheckAllXinfaLevel", sceneId, selfId, 15 ) == 0  then	-- zchw
   	BeginEvent(sceneId)
			local strText = format("NhiÆm v¯ n‡y yÍu c•u 6 lo’i t‚m ph·p ´u ’t c§p 15.")
	...
	if ret > 0  then
		...
		if ret <= 0 then
			local strText = format("#YNhßt k˝ nhiÆm v¯ c¸a c·c h’ „ •y")
		...
		local strText = format("#YTi™p nhßn nhiÆm v¯: Th∂i tu±i trÎ ngÙng cu∞ng")
		Msg2Player(sceneId, selfId, strText,MSG2PLAYER_PARA)
	 	BeginEvent(sceneId)
			AddText(sceneId, strText)
	  EndEvent(sceneId)
...
function x212119_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	if x212119_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		Msg2Player( sceneId, selfId,"#YHo‡n th‡nh nhiÆm v¯: Th∂i tu±i trÎ ngÙng cu∞ng",MSG2PLAYER_PARA )
	end
end
