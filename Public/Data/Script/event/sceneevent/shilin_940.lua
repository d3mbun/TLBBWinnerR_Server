...
function x212124_OnAccept( sceneId, selfId, targetId )
	...
	-- ¼ì²éÍæ¼ÒµÄÐÄ·¨ÊÇ²»ÊÇ´ïµ½ÁËÒªÇó£¨Ç°ÁùÖÖÐÄ·¨µÈ¼¶´óÓÚµÈÓÚ55¼¶£©
	...
	if x212124_g_Step ~= 0  and x212124_g_Step ~= 2   then
		BeginEvent(sceneId)
			AddText(sceneId, "Møc tiêu ðang b§n, xin ðþi giây lát!")
		EndEvent()
		DispatchMissionTips(sceneId, selfId)
		return
	end
...
function x212124_OnSceneTimer(sceneId, selfId)
	...
	elseif nStep == 2   then
		...
		if LuaFnGetCurrentTime() - x212124_g_StartTime > 5   then
			--CallScriptFunction((200060), "Paopao",sceneId, "ÑîÌúÐÅ", "Ê¯ÁÖ", "³ö·¢£¬³ö·¢!!!")
			--nStep = 3
	...
	elseif nStep == 3   then
		...
		if nHaveMonster == 0  then
			PrintStr("Can't find Dß½ng Thiªt Tín")
			return 
		end
		--µÃµ½»¤ËÍÑ²ÂßÂ·Ïß
