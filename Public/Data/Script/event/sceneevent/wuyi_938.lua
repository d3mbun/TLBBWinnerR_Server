...
x212123_g_MisItemId = 40004420 --«˙∂Àµƒ–≈∫Ø
...
function x212123_OnAccept( sceneId, selfId, targetId )
	...
	-- ºÏ≤ÈÕÊº“µƒ–ƒ∑® «≤ª «¥ÔµΩ¡À“™«Û£®«∞¡˘÷÷–ƒ∑®µ»º∂¥Û”⁄µ»”⁄25º∂£©
	if CallScriptFunction( 212120, "CheckAllXinfaLevel", sceneId, selfId, 25 ) == 0  then -- zchw
   	BeginEvent(sceneId)
			local strText = format("NhiÆm v¯ n‡y yÍu c•u 6 lo’i t‚m ph·p ´u ’t c§p 25.")
	...
	if ret > 0  then
		...
		Msg2Player(sceneId, selfId,"#YTi™p nhßn nhiÆm v¯: Nhßn sÒ ¸y th·c",MSG2PLAYER_PARA)
	 	BeginEvent(sceneId)
			AddText(sceneId, "#YTi™p nhßn nhiÆm v¯: Nhßn sÒ ¸y th·c")
	  EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
...
function x212123_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	if x212123_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		Msg2Player( sceneId, selfId,"#YHo‡n th‡nh nhiÆm v¯: Nhßn sÒ ¸y th·c",MSG2PLAYER_PARA )
	end
end
