--!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
--¸Ã½Å±¾ÎÄ¼þÖ»¹©Nhi®m vø sß môn¶ø×ö, ÆäËûÈÎÎñÈçÒªÒýÓÃ, ºó¹û×Ô¸º!
--!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
x500501_g_strHelpFinishedText = "  Ð°ng môn g£p nÕn, nên tß½ng trþ, nhi®m vø cüa ngß½i ta ðã s¡p xªp v¾i ð°ng môn khác ði hoàn thành.#r#GNhi®m vø hoàn thành#W!"
...
function x500501_ACT_Duanwu( sceneId, selfId, iDayHuan ) --¶ËÎç»î¶¯
	...
	if AddRet > 0 then
		...
		local Gemable = random(100)  -- 1/5 ðÕt ðßþc±¦Ê¯
		if Gemable > 20 then
			return
		end
		...
		if Duanwu_GemCount >= Duanwu_MaxGemCount then
			return     --±¦Ê¯ÊýÁ¿³¬¹ýÁËÃ¿ÌìËÍ³ötoÕ ðµ ×î´óÊýÁ¿
		end
...
--¸Ä³É²»¿ÉÄÜThöa mãntoÕ ðµ Ìõ¼þÀ´HuÖ böLinh Thúµ¤toÕ ðµ ½±Àø,by zhangqiang 2009.6.18
--		if GetLevel(sceneId, selfId) >= 50 and iDayHuan == 20 then --50c¤pÒÔÉÏ²Å ðÕt ðßþc,Ö»ÓÐµÚmµt cái20»·²Å¸ø,¶ø²»ÐúngÃ¿20»·¶¼¸ø
		if 0 > 1 then
...
--					-- ·¢ÊÀ½ç¹«¸æ
--					local transfer = GetItemTransfer(sceneId,selfId,0)
--					local str = format("#W#{_INFOUSR%s}#cffffcc hôm nay vì lúc c¯ g¡ng hoàn thành 20 l¥n nhi®m vø sß môn, do quá m®t möi ðã ng°i nghï dß¾i ð¤t, t× trong túi r½i ra 1 viên #W#{_INFOMSG%s} l¤p lánh.",GetName(sceneId,selfId), transfer)
--					----¹«¸æTinh¼ò,È¥³ý´Ë¹«¸æ
--					--BroadMsgByChatPipe(sceneId, selfId, str, 4)
--					
--				end
				...
				--¸ø×Ô¼º·¢ËÍ ði¬mµ½ÎïÆ·toÕ ðµ ÏûÏ¢....
				local strMsg = format("Các hÕ ðã ðÕt ðßþc #c00FFFF#{_ITEM%d}#W.", x500501_g_ItemId[randomIndex])
				Msg2Player(sceneId, selfId, strMsg, MSG2PLAYER_PARA)
  ...
  -- ========================================================
	-- Ê¥µ®»î¶¯ Íê³ÉNhi®m vø sß môn ðÕt ðßþc·û½Ú
...
--	Msg2Player(  sceneId, selfId,"Äã ði¬mµ½ÁË#Y1#W ði¬mÃÅÅÉ¹±Ï×¶È." ,MSG2PLAYER_PARA )
...
--	Msg2Player(  sceneId, selfId,"Äã ði¬mµ½ÁË#Y2#W ði¬mÃÅÅÉ¹±Ï×¶È." ,MSG2PLAYER_PARA )
--else --mod(iDayHuan, 10) must return 0
--	SetHumanMenpaiPoint(sceneId, selfId, GetHumanMenpaiPoint(sceneId, selfId)+3	)
--	Msg2Player(  sceneId, selfId,"Äã ði¬mµ½ÁË#Y3#W ði¬mÃÅÅÉ¹±Ï×¶È." ,MSG2PLAYER_PARA )
--end

	--Êµ¼Ê½±Àø¹±Ï×¶È=(Íæ¼ÒµÈc¤p-»ù´¡µÈc¤p)* µÈc¤pÓ°Ïì²ÎÊý + »ù´¡¹±Ï×¶È
	...
	--//////////////////////////////////////////////////
	--ÓÐ10%toÕ ðµ ¼¸ÂÊ ðÕt ðßþcmµt cáiµÍc¤p±¦Ê¯ºÏ³É·û
	--AAA½ñÈÕÎªÊ¦ÃÅ×öÈÎÎñÓÂÍùÖ±Ç°,·Ü²»¹ËÉí,TÕi Íê³ÉµÚ20´Î
	--¶à±¶Ê±,ÖÕÒòÌ«ÀÛmµt õÓË¤µ¹TÕi Â·±ß,ÅÀÆðÀ´toÕ ðµ Ê±ºò,Á³ÉÏ
	--¾ÓÈ»Õ³×Åmµt ÕÅÖå°Í°ÍtoÕ ðµ [µÍc¤p±¦Ê¯ºÏ³É·û].
	--ID: 30900015
	--?	Íæ¼ÒµÈc¤p±ØÐë >=40c¤p
	...
	local MijiActive = 1
	local AwardID = 30505078    --ÌìÊé²Ð¾ítoÕ ðµ ID
	local AwardCyc = 20         --½±Àø»·Êý( ×ö¶àÉÙ´ÎÈÎÎñ»á¸ømµt ´Î½±Àø )
	...
	--	x500501_g_Exp = 2400*(Level+4) * 10 * l_Exp / 120						--µÈc¤p+»·Êýº¯Êý,ÊÜKinh nghi®mµ÷½Ú³£ÊýtoÕ ðµ Ó°Ïì
		--x500501_g_Money = 2400*(Level+4) * 10 * l_Money /120						--µÈc¤p+»·Êýº¯Êý,ÊÜKinh nghi®mµ÷½Ú³£ÊýtoÕ ðµ Ó°Ïì
	--else
	--	x500501_g_Exp = 2400*(Level+4) * mod(x500501_g_MissionRound,10) * l_Exp / 120						--µÈc¤p+»·Êýº¯Êý,ÊÜKinh nghi®mµ÷½Ú³£ÊýtoÕ ðµ Ó°Ïì
		--x500501_g_Money = 2400*(Level+4) * mod(x500501_g_MissionRound,10) * l_Money /120						--µÈc¤p+»·Êýº¯Êý,ÊÜKinh nghi®mµ÷½Ú³£ÊýtoÕ ðµ Ó°Ïì
	--end

	-- 1+µÈc¤pÓ°Ïì*µ±Ç°µÈc¤p/10 {µÈc¤pÓ°Ïì=0.035,Ó°ÏìÖ¸Êý=1}
	...
	--	x500501_g_Exp = 2400*(Level+4) * l_Exp / 120 + x500501_g_Exp						--11~20»·ÈÎÎñ,Ã¿»·¶îÍâÔö¼Ómµt ¶¨Kinh nghi®mÔöÁ¿,½ðÇ®Ã»ÓÐÔöÁ¿
	--end
	...
	--ÏÔÊ¾¶Ô»°¿ò
	BeginEvent(sceneId)
		--Èç¹ûÐúngÍ¬ÃÅÏàÖúÍê³ÉtoÕ ðµ ....Ôò¼ÓÈëÏà¹Ø¶Ô»°....
	...
	--Msg2Player(  sceneId, selfId,"Äã ði¬mµ½ÁË#{_MONEY" .. tostring(x500501_g_Money) .. "}" ,MSG2PLAYER_PARA )
	--Msg2Player(  sceneId, selfId,"Äã ði¬mµ½ÁË" .. tostring(x500501_g_Exp) .. " ði¬mKinh nghi®mÖµ" ,MSG2PLAYER_PARA )

	--TÕi mµt ¶¨Ìõ¼þÏÂ¿ÉÒÔ ði¬mµ½¶îÍâtoÕ ðµ ½±Àø
...
function x500501_CheckAccept_Necessary( sceneId, selfId )
	...
	if iDayTime == CurDaytime then
		--Èç¹ûµ±Ìì×ö¸ÃS¯ l¥n nhi®m vø ´ïµ½60Ôò²»ÔÊÐíÔÙ½Ó
		if iDayHuan >= 60 then
			return -2;
		end
...
function x500501_OnMissionCheck_NecessaryEx( sceneId, selfId, index1, index2, index3, Needindex )
	...
	for i = 1, getn( indices ) do
		if indices[i] and indices[i] >=0 and indices[i] < 80 then	-- ºÏ·¨toÕ ðµ ±³°ü¸ñ×Ó
