...
function x500503_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if IsHaveMission( sceneId, selfId, x500503_g_MissionId ) > 0 then				
		...
		BeginEvent(sceneId)
			...
			AddText( sceneId, "#r#GNhi®m vø này tß½ng Ñng v¾i th¥n khí c¤p: "..shenqilvl )
		EndEvent( )
...
--			ret2 = EndAddItem( sceneId, selfId )
			...
			--ÏÔÊ¾¶Ô»°¿ò
			BeginEvent( sceneId )	
				AddText( sceneId, "  Làm t¯t l¡m, · ðây có Tàn khuyªt th¥n tiªt, ðßa cho ngß½i xem nhß ph¥n thß·ng." )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )			
			...
			BeginEvent(sceneId)
			strText = "Nh§n thß·ng th¤t bÕi hãy nh§n lÕi mµt l¥n næa"
			AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return 0
		...
		if IsHaveMission(sceneId, humanObjId, x500503_g_MissionId) > 0 then
			...
			if GetMissionParam(sceneId, humanObjId, misIndex, 0) <=0  then
				...
				if ((PlayerLevel - MonsterLevel >= 0) and (PlayerLevel - MonsterLevel < 10))
					...
					BeginEvent(sceneId)
						local str = format("Ðã giªt chªt quái v§t yêu c¥u: %d/%d",killedCount, demandKillCount )						
						AddText(sceneId, str)
					EndEvent(sceneId)
...
function x500503_OnMissionCheck( sceneId, selfId, npcid, scriptId, index1, index2, index3, petindex )
		...
		if index < 100 then
			...
			if accres > 0 then
				...
				if	ret > 0 then
					...
					BeginEvent(sceneId)			
						local strText = format("Kh¤u tr× vû khí th¤t bÕi")
						AddText(sceneId,strText)									
					EndEvent(sceneId)
	...
	x500503_NotifyTip( sceneId, selfId, "Nh§n nhi®m vø th¤t bÕi: vû khí không hþp l®" )
...
function x500503_GetItemDetailInfo(itemId)
	...
	if itemId == -1 then
		local strText = format("%s v§t ph¦m · 'EquipBase.txt' không tìm th¤y!!", itemName)		
	end
