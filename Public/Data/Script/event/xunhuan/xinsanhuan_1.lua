...
x050220_g_TakeTimes = 5											-- Ã¿Ìì×î¶àLînh ´ÎÊý
--**********************************
-- ÈÎÎñÈë¿Úº¯Êý
--**********************************
		...
		if otherflag == 0 then --Áímµt cáibossÃ»ÓÐËÀ
			for i=0, nMonsterNum-1 do
				...
				if GetName( sceneId, MonsterId ) == x050220_g_NiuQi then
					...
					MonsterTalk(sceneId,MonsterId,"Huy«n Lôi Pha", "ÐÕi ca, huynh ð® báo thù cho huynh!")
				end
			end
		else
			for i=0, nMonsterNum-1 do
		...
		if otherflag == 0 then --Áímµt cáibossÃ»ÓÐËÀ
			for i=0, nMonsterNum-1 do
				...
				if GetName( sceneId, MonsterId ) == x050220_g_NiuQu then
					...
					MonsterTalk(sceneId,MonsterId,"Huy«n Lôi Pha", "Huynh ð®, ðÕi ca báo thù cho huynh!")
				end
			end
		else
			for i=0, nMonsterNum-1 do
...
--	local LevelGap = LuaFnGetCopySceneData_Param( sceneId, CopyScene_LevelGap )
	...
	elseif TickCount == 1 then									--¿ªÊ¼toÕ ðµ Ê±ºòË£¹Ö.µØ ði¬mA»áÓÐ15cáiÐ¡¹ÖºÍBOSS1Ngßu Khúc,µØ ði¬mB»áÓÐ20cáiÐ¡¹ÖºÍBOSS2Ngßu KÏ,µØ ði¬mC»áÓÐ25cáiÐ¡¹Ö.
		...
		if monsternum >= 60 and iskillNiuQu == 1 and iskillNiuQi == 1 and isBossWang == 0 then
			...
			MonsterTalk(sceneId,monsterID,"Huy«n Lôi Pha","Oa nha nha, Ngß¶i nào dám s¤m ta huy«n lôi pha, mau ðªn nh§n l¤y cái chªt!")
