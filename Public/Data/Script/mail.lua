...
function x888889_NoScriptTypeMail( sceneId, selfId, scripttype)
	...
	elseif scripttype == MAIL_SHITUPRIZE then
		x888889_NotifyFailBox( sceneId, selfId, -1, "   Các hÕ hi®n th¶i không có ph¥n thß·ng không th¬ lînh nh§n. Hãy ði ð¯c thúc ð° ð® cüa các hÕ thång c¤p ði!" )
...
function x888889_Mail_ShiTuPrize( sceneId, selfId, param0, param1, param2, param3 )
	...
	if 40 == plevel then
		if 10 < c40+1 then
			x888889_NotifySystemMsg(sceneId,selfId,"Không th¬ nh§n sß phø ph¥n thß·ng  th¤t bÕi, ðã ðÕt t¾i thßþng hÕn.")
			return
		end
	elseif 50 == plevel then
		if 10 < c50+1 then
			x888889_NotifySystemMsg(sceneId,selfId,"Nh§n l¤y ph¥n thß·ng ð£t bi®t c¤p cho danh sß th¤t bÕi, ðã ðÕt t¾i thßþng hÕn.")
			return
		end
	end
	...
	if 1 == ret then
		...
		x888889_NotifySystemMsg(sceneId,selfId,"Nh§n l¤y ph¥n thß·ng thành công")
		--ÔÚ³é½±ÈÕÖ¾ÖÐ¼ÇÂ¼....
		...
		x888889_NotifySystemMsg(sceneId,selfId,"Không gian ô nhi®m vø ðÕo cø không ðü, nh§n lînh th¤t bÕi")
		return
	end
