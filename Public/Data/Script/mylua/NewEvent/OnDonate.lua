x045006_g_scriptId = 045006

function x045006_OnPlayerDonate(sceneId,selfId,Donate)

	local TopDonate10 = x045006_GetValue2Array(sceneId,10)
	if CharLevel > TopDonate10 then
		...
		x045006_Top10Process(sceneId,CharStr,CharName,Donate,BonusInfo)
	end
end

function x045006_GetValue2Array(sceneId,nValue)
	local handle = openfile('Var/DuaTop/TopDonate.txt', "r")
	local flag = {};
...
function x045006_Top10Process(sceneId,nGuid,nName,nValue,nTime)
	--Sort Data--
	local TopArray = x045006_GetValue2Array(sceneId,0)
	local NewTopArray = {}
	...
	--Save Data--
	local handle = openfile('Var/DuaTop/TopDonate.txt', "wb")
	if handle then
		for i = 1, 10 do
...
function x045006_Syn2Client(sceneId,selfId,type)
	if GetMissionData(sceneId,selfId,CLICK_TIME_LIMIT) >= LuaFnGetCurrentTime() - 4 then
		x045006_Noitice(sceneId,selfId,"Vui lòng nh¤p chuµt ch§m rãi, t× t¯n!")
		return
	end
	
	SetMissionData(sceneId,selfId,CLICK_TIME_LIMIT,LuaFnGetCurrentTime())
	x045006_Noitice(sceneId,selfId,"C§p nh§t dæ li®u hoàn t¤t!")
	...
	DispatchUICommand(sceneId,selfId,1023002)
end

function x045006_Noitice(sceneId,selfId,str)
	BeginEvent(sceneId)
		AddText(sceneId,str)
	EndEvent(sceneId)
