x045007_g_scriptId = 045007

function x045007_OnPlayerLevelUp(sceneId,selfId,CharLevel)
	
	local TopLevel10 = x045007_GetValue2Array(sceneId,10)
	if CharLevel > TopLevel10 then
		...
		x045007_Top10Process(sceneId,CharStr,CharName,CharLevel,BonusInfo)
	end
end

function x045007_GetValue2Array(sceneId,nValue)
	local handle = openfile('Var/DuaTop/TopUseKNB.txt', "r")
	local flag = {};
...
function x045007_Top10Process(sceneId,nGuid,nName,nValue,nTime)
	--Sort Data--
	local TopArray = x045007_GetValue2Array(sceneId,0)
	local NewTopArray = {}
	...
	--Save Data--
	local handle = openfile('Var/DuaTop/TopUseKNB.txt', "wb")
	if handle then
		for i = 1, 10 do
...
function x045007_Syn2Client(sceneId,selfId,type)
	if GetMissionData(sceneId,selfId,CLICK_TIME_LIMIT) >= LuaFnGetCurrentTime() - 4 then
		x045007_Noitice(sceneId,selfId,"Vui lòng nh¤p chuµt ch§m rãi, t× t¯n!")
		return
	end
	
	SetMissionData(sceneId,selfId,CLICK_TIME_LIMIT,LuaFnGetCurrentTime())
	x045007_Noitice(sceneId,selfId,"C§p nh§t dæ li®u hoàn t¤t!")
	...
	DispatchUICommand(sceneId,selfId,1024002)
end

function x045007_Noitice(sceneId,selfId,str)
	BeginEvent(sceneId)
		AddText(sceneId,str)
	EndEvent(sceneId)
