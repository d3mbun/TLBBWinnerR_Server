...
--ÊÂ¼þ½»»¥Èë¿Ú
--**********************************
function x100010_OnDefaultEvent( sceneId, selfId, bagIndex )
...
function x100010_OnDeplete( sceneId, selfId )
	...
	if guildid == -1 then
		x100010_ShowNotice( sceneId, selfId, "Các hÕ chßa gia nh§p bang hµi, tÕm th¶i không th¬ sØ døng." )
		return 1
	end
...
function x100010_OnActivateOnce( sceneId, selfId )
	...
	CityChangeAttr( sceneId, selfId, GUILD_AGR_LEVEL, 200 )
	BeginEvent(sceneId)
       x100010_ShowNotice( sceneId, selfId, "Chúc m×ng bang hµi các hÕ ðã tång ðµ Nông nghi®p 200 ði¬m." )
	return 1;
end
...
function x100010_ShowNotice( sceneId, selfId, strNotice)
	BeginEvent( sceneId )
