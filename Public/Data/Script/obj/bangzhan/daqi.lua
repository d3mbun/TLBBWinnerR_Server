...
x402301_g_OpenFlagSelfIDIndex = 10						--µ±Ç°ÕýÔÚ²Ù×÷ÆìÌ¨µÄselfidÔÚ32¸ö¸±±¾Êý¾ÝÖÐµÄË÷Òý
x402301_g_OpenFlagStartTime = 11							--µ±Ç°ÕýÔÚ²Ù×÷ÆìÌ¨µÄ¿ªÊ¼Ê±¼äÔÚ32¸ö¸±±¾Êý¾ÝÖÐµÄË÷Òý
x402301_g_FlagRemainedTime = 12								--ÆìÖÄÉýÆðÒÔºóµÄÊ£ÓàÊ±¼äÔÚ32¸ö¸±±¾Êý¾ÝÖÐµÄË÷Òý

x402301_g_GuildPoint_GetFlag = 4							--»ý·Ö¹æÔòË÷Òý£¬±íGuildWarPoint.txtµÄID

--°ïÕ½Êý¾ÝË÷Òý£¬¶ÔÓ¦³ÌÐòÖÐµÄ enum GUILDWAR_INT_ARRAY
x402301_g_A_FlagNumIndex = 16									--A°ïÕ¼Áì´óÆì´ÎÊýÔÚ°ïÕ½Êý×éÖÐµÄË÷Òý
x402301_g_B_FlagNumIndex = 17									--B°ïÕ¼Áì´óÆì´ÎÊýÔÚ°ïÕ½Êý×éÖÐµÄË÷Òý

--¸öÈË»ý·ÖÊý¾ÝË÷Òý£¬¶ÔÓ¦³ÌÐòenum GUILDWAR_INT_INDEX
x402301_g_Human_FlagIndex = 3									--¸öÈËÕ¼Áì´óÆì´ÎÊýË÷Òý
...
x402301_g_B_FlagName = "Xuy Vßu Chiªn KÏ"
...
function x402301_OnActivateConditionCheck( sceneId, selfId, activatorId )
	...
	if guildid ~= Aguildid and guildid ~= Bguildid then
		BeginEvent(sceneId)
			AddText(sceneId,"Ngß½i không n¢m trong bang phái")
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,activatorId)
		return 0
	end
	...
	if OpenFlagSelfId ~= 0 then --ÓÐÈËÕýÔÚÒýµ¼¼ÀÆì
	...
	LuaFnSetCopySceneData_Param( sceneId, x402301_g_OpenFlagSelfIDIndex, activatorId )	-- µ±Ç°ÕýÔÚ²Ù×÷ÆìÌ¨µÄselfid
	LuaFnSetCopySceneData_Param( sceneId, x402301_g_OpenFlagStartTime, NowTime )	-- µ±Ç°ÕýÔÚ²Ù×÷ÆìÌ¨µÄ¿ªÊ¼Ê±¼ä
...
function x402301_OnActivateEffectOnce( sceneId, selfId, activatorId )
	...
	if guildid == Aguildid then
		...
		LuaFnSetCopySceneData_Param( sceneId, x402301_g_FlagRemainedTime, x402301_g_FlagTime )	--ÆìÖÄÉýÆðÒÔºóµÄÊ£ÓàÊ±¼ä
		...
		local msg = LuaFnGetGuildName(sceneId, activatorId).."#{BHXZ_081103_125}"..GetName(sceneId, activatorId).." ðã tång lên "..x402301_g_A_FlagName.."."
		for i=0, membercount-1 do
	...
	elseif guildid == Bguildid then
		...
		local msg = LuaFnGetGuildName(sceneId, activatorId).."#{BHXZ_081103_125}"..GetName(sceneId, activatorId).."ÒÑ¾­ÉýÆðÁË"..x402301_g_B_FlagName.."."
		for i=0, membercount-1 do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, mems[i] ) == 1 then
				x402301_NotifyFailTips(sceneId, mems[i], msg)
				Msg2Player(sceneId, mems[i], msg, MSG2PLAYER_PARA)
	...
	LuaFnSetCopySceneData_Param( sceneId, x402301_g_OpenFlagSelfIDIndex, 0 )	-- µ±Ç°ÕýÔÚ²Ù×÷ÆìÌ¨µÄselfid
	LuaFnSetCopySceneData_Param( sceneId, x402301_g_OpenFlagStartTime, 0 )	-- µ±Ç°ÕýÔÚ²Ù×÷ÆìÌ¨µÄ¿ªÊ¼Ê±¼ä
