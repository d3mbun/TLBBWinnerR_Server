...
-- needLevel: ÑÐ¶Á´ËÊéÐèÒªµÄÏàÓ¦ÈËÎï¼¶±ð£¬ -1 ±íÊ¾Ã»ÓÐÒªÇó
-- needXinfa: ÑÐ¶Á´ËÊéÐèÒªµÄÏàÓ¦ÐÄ·¨£¬ -1 ±íÊ¾Ã»ÓÐÒªÇó
-- needXinfaLevel: ÑÐ¶Á´ËÊéÐèÒªÏàÓ¦ÐÄ·¨µÄµÈ¼¶£¬ -1 ±íÊ¾Ã»ÓÐÒªÇó
...
x338000_g_SkillBooks[30308099] = { type = 2, id = 782, menpaiId = MP_GUSU, needLevel = 45, needXinfa = 70, needXinfaLevel = 1, specialEffectID = 18 }
x338000_g_SkillBooks[30308100] = { type = 2, id = 781, menpaiId = MP_GUSU, needLevel = 65, needXinfa = 70, needXinfaLevel = 1, specialEffectID = 18 }
--bt 80
...
x338000_g_SkillBooks[30308110] = { type = 1, id = 71, menpaiId = MP_GUSU, needLevel = 80, needXinfa = -1, needXinfaLevel = 1, specialEffectID = 18 }
--skill
...
x338000_g_SkillBooks[30308120] = { type = 2, id = 786, menpaiId = MP_GUSU, needLevel = 80, needXinfa = 71, needXinfaLevel = 1, specialEffectID = 18 }
...
x338000_g_SkillBooks[30308130] = { type = 2, id = 787, menpaiId = MP_GUSU, needLevel = 80, needXinfa = 71, needXinfaLevel = 1, specialEffectID = 18 }

x338000_g_SkillBooks[30308153] = { type = 2, id = 784, menpaiId = MP_GUSU, needLevel = 45, needXinfa = 71, needXinfaLevel = 1, specialEffectID = 18 }
x338000_g_SkillBooks[30308154] = { type = 2, id = 785, menpaiId = MP_GUSU, needLevel = 80, needXinfa = 71, needXinfaLevel = 1, specialEffectID = 18 }
...
function x338000_OnConditionCheck( sceneId, selfId )
	...
	local Phai = {"Thiªu Lâm", "Minh Giáo", "Cái Bang", "Võ Ðang", "Nga My", "Tinh Túc", "Thiên Long", "Thiên S½n", "Tiêu Dao", "Không Có", "Mµ Dung", "Ðß¶ng Môn", "QuÖ C¯c"}
	if -1~=skillBook.menpaiId then
	 if menpai ~= skillBook.menpaiId then
		 x338000_NotifyFailTips( sceneId, selfId, "Ngß½i không phäi là ð® tØ " ..Phai[skillBook.menpaiId+1] .. ", không th¬ h÷c ðßþc " .. x338000_g_TypeNames[skillBook.type] .. "." )
		 return 0
	 end
	end
	...
	if skillBook.needXinfa ~= -1 then
		...
		if xinfaLevel < skillBook.needXinfaLevel then
			-- ÐèÒª²âÊÔÕâ¸öµØ·½£¬¿çÐÐ¶øÇÒÃ»ÓÐ·ÖºÅ
			x338000_NotifyFailTips( sceneId, selfId, "C¥n tâm pháp#{_XINFA" .. skillBook.needXinfa .. "} "
				.. skillBook.needXinfaLevel .. " C¤p ðµ hi®n tÕi chua th¬ h÷c ðßþc " .. xinfaLevel .. " ." )
			return 0
		end
	end
	...
	if skillBook.id == 781 then				-- Yªu Quyªt
			AddSkill( sceneId, selfId, 788 )
			AddSkill( sceneId, selfId, 789 )
			AddSkill( sceneId, selfId, 790 )
			AddSkill( sceneId, selfId, 791 )
			AddSkill( sceneId, selfId, 792 )
			AddSkill( sceneId, selfId, 793 )
			AddSkill( sceneId, selfId, 794 )
			AddSkill( sceneId, selfId, 795 )
			AddSkill( sceneId, selfId, 796 )
	end
...
function x338000_OnActivateOnce( sceneId, selfId )
	...
	if skillBook.type == 1 then					-- ÐÄ·¨
		if HaveXinFa( sceneId, selfId, skillBook.id ) > 0 then
			x338000_NotifyFailTips( sceneId, selfId, "Ngß½i phäi h÷c tâm pháp này" )
			return 0
		else
