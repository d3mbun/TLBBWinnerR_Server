...
function x805014_UpdateEventList( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		if Humanguildid == cityguildid then
			...
			if IsHaveMission( sceneId, selfId, 1106 ) == 1 then
				AddNumText( sceneId, x805014_g_scriptId, "#GHoàn T¤t Nhi®m Vø Phát Tri¬n", 4, 1001 )
			end
...
function x805014_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 1001 then
		if IsHaveMission( sceneId, selfId, 1106 ) <= 0 then --Chua Nhan Nhiem Vu
			BeginEvent( sceneId )
				AddText( sceneId, "Các hÕ vçn chßa nh§n Nhi®m vø Phát Tri¬n nào cä?" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			local misIndex = GetMissionIndexByID(sceneId, selfId, 1106)
			...
			if misIndexComplete >= 1 then
				...
				BeginEvent( sceneId )
					...
					AddNumText( sceneId, x805014_g_scriptId, "Ta dùng #GNgân Lßþng", 6, 10011 )
					AddNumText( sceneId, x805014_g_scriptId, "Ta dùng #GC¯ng Hiªn", 6, 10012 )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
			end
		end
	...
	if GetNumText() == 10011 then
		...
		if CostMoney(sceneId,selfId,NeedMoney) == -1 then
			BeginEvent( sceneId )
				AddText( sceneId, "Ngß½i không có ðü #{_MONEY"..NeedMoney.."} ð¬ hoàn t¤t nhi®m vø Phát Tri¬n!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			local misIndex = GetMissionIndexByID(sceneId, selfId, 1106)
			...
			CallScriptFunction( 600007, "OnSubmit", sceneId, selfId, targetId, 99999 )
			
			BeginEvent( sceneId )
				AddText( sceneId, "Các hÕ ðã hoàn t¤t nhi®m vø Phát Tri¬n!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
	...
	if GetNumText() == 10012 then
		...
		if NeedCH > CurrCH then
			BeginEvent( sceneId )
				AddText( sceneId, "Ngß½i không có ðü #G"..NeedCH.."#W ði¬m #GC¯ng Hiªn #Wð¬ hoàn t¤t nhi®m vø Phát Tri¬n!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
...
function x805014_BingZhenXiGua(sceneId, selfId, targetId)
	...
	local strChatMessage = "#{_INFOUSR"..selfName.."}#P h¾n h· t× trong tay#G Chu Thª hæu[129, 100]#P l¤y 1 miªng#Y#{_INFOMSG" .. szTransferItem .. "}#P, ån 1 miªng vào mát t× ð¥u xu¯ng ðªn bàn chân!";
