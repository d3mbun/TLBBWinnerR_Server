	...
	BeginEvent( sceneId )
		if Humanguildid == cityguildid then
			...
			if IsHaveMission( sceneId, selfId, 1110 ) == 1 then
				AddNumText( sceneId, x805013_g_scriptId, "#GHoàn T¤t Nhi®m Vø Khuªch Trß½ng ", 4, 1001 )
			end
...
function x805013_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 1001 then
		if IsHaveMission( sceneId, selfId, 1110 ) <= 0 then --Chua Nhan Nhiem Vu
			BeginEvent( sceneId )
				AddText( sceneId, "Các hÕ vçn chßa nh§n Nhi®m vø Khuªch Trß½ng nào cä?" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			...
			if misIndexComplete >= 1 then
				...
				BeginEvent( sceneId )
					...
					AddNumText( sceneId, x805013_g_scriptId, "Ta dùng #GNgân Lßþng", 6, 10011 )
					AddNumText( sceneId, x805013_g_scriptId, "Ta dùng #GC¯ng Hiªn", 6, 10012 )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
			end
		end
	...
	if GetNumText() == 10011 then
		...
		if CostMoney(sceneId,selfId,NeedMoney) == -1 then
			BeginEvent( sceneId )
				AddText( sceneId, "Ngß½i không có ðü #{_MONEY"..NeedMoney.."} ð¬ hoàn t¤t nhi®m vø Khuªch Trß½ng!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			local misIndex = GetMissionIndexByID(sceneId, selfId, 1110)
			...
			BeginEvent( sceneId )
				AddText( sceneId, "Các hÕ ðã hoàn t¤t nhi®m vø Khuªch Trß½ng!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		...
		if GetTime2Day() == 20200927 then
			if GetHour() <= 5 then
				...
				if NeedCH > CurrCH then
					BeginEvent( sceneId )
						AddText( sceneId, "Ngß½i không có ðü #G"..NeedCH.."#W ði¬m #GC¯ng Hiªn #Wð¬ hoàn t¤t nhi®m vø Khuªch Trß½ng!" )
					EndEvent( sceneId )
					DispatchEventList( sceneId, selfId, targetId )
				else
				...
				BeginEvent( sceneId )
					AddText( sceneId, "ChÑc nång không khä døng!" )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
			end
		else
