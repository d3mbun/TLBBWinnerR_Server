...
--		if CityGetSelfCityID( sceneId, selfId ) == sceneId then
			...
			if Humanguildid == 2 or Humanguildid == 9 or Humanguildid == 6 then
				if GetGuildPos(sceneId, selfId) >= 9 then
					AddNumText( sceneId, x805009_g_scriptId, "#GNh§n Ð°ng Phøc Bang Hµi", 4, 1003 )
				end
			end
			
			if IsHaveMission( sceneId, selfId, 1111 ) == 1 then --Chua Nhan Nhiem Vu
				AddNumText( sceneId, x805009_g_scriptId, "#GHoàn T¤t Nhi®m Vø Xây Dñng", 4, 1001 )
			end
			
			if IsHaveMission( sceneId, selfId, 1113 ) == 1 then --Chua Nhan Nhiem Vu
				AddNumText( sceneId, x805009_g_scriptId, "#GHoàn T¤t Nhi®m Vø Nghiên CÑu", 4, 1002 )			
			end
			...
			AddNumText( sceneId, x805009_g_scriptId, "#{BGCH_8829_02}", 11, 25 )--¹ØÓÚ¶mµt »°ï¹±ÅÆ
			AddNumText( sceneId, x805009_g_scriptId, "Xin tr· thành Bang chü", 11, 23 )
 ...
 --»Øµ½´óÀí
	...
	if GetNumText() == 1001 then
		if IsHaveMission( sceneId, selfId, 1111 ) <= 0 then --Chua Nhan Nhiem Vu
			BeginEvent( sceneId )
				AddText( sceneId, "Các hÕ vçn chßa nh§n Nhi®m vø Xây Dñng nào cä?" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			...
			if misIndexComplete >= 1 then
				...
				BeginEvent( sceneId )
					...
					AddNumText( sceneId, x805009_g_scriptId, "Ta dùng #GNgân Lßþng", 6, 10011 )
					AddNumText( sceneId, x805009_g_scriptId, "Ta dùng #GC¯ng Hiªn", 6, 10012 )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
			end
		end
	...
	if GetNumText() == 10011 then
		...
		if CostMoney(sceneId,selfId,NeedMoney) == -1 then
			BeginEvent( sceneId )
				AddText( sceneId, "Ngß½i không có ðü #{_MONEY"..NeedMoney.."} ð¬ hoàn t¤t nhi®m vø Xây Dñng!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			local misIndex = GetMissionIndexByID(sceneId, selfId, 1111)
			...
			BeginEvent( sceneId )
				AddText( sceneId, "Các hÕ ðã hoàn t¤t nhi®m vø Xây Dñng!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
	...
	if GetNumText() == 10012 then
		...
		if NeedCH > CurrCH then
			BeginEvent( sceneId )
				AddText( sceneId, "Ngß½i không có ðü #G"..NeedCH.."#W ði¬m #GC¯ng Hiªn #Wð¬ hoàn t¤t nhi®m vø Xây Dñng!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
	...
	if GetNumText() == 1002 then
		if IsHaveMission( sceneId, selfId, 1113 ) <= 0 then --Chua Nhan Nhiem Vu
			BeginEvent( sceneId )
				AddText( sceneId, "Các hÕ vçn chßa nh§n Nhi®m vø Nghiên CÑu nào cä?" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			...
			if misIndexComplete >= 1 then
				...
				BeginEvent( sceneId )
					...
					AddText( sceneId, "Ði¬m phí C¯ng Hiªn: "..(Guild_LV*2).."#G CH" )
					--AddText( sceneId, "#{_MONEY10000}" )
					AddNumText( sceneId, x805009_g_scriptId, "Ta dùng #GNgân Lßþng", 6, 10021 )
					AddNumText( sceneId, x805009_g_scriptId, "Ta dùng #GC¯ng Hiªn", 6, 10022 )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
			end
		end
	...
	if GetNumText() == 10021 then
		...
		if CostMoney(sceneId,selfId,NeedMoney) == -1 then
			BeginEvent( sceneId )
				AddText( sceneId, "Ngß½i không có ðü #{_MONEY"..NeedMoney.."} ð¬ hoàn t¤t nhi®m vø Nghiên CÑu!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			local misIndex = GetMissionIndexByID(sceneId, selfId, 1113)
			...
			BeginEvent( sceneId )
				AddText( sceneId, "Các hÕ ðã hoàn t¤t nhi®m vø Nghiên CÑu!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
			LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 148, 0)
		end
	...
	if GetNumText() == 10022 then
		...
		if NeedCH > CurrCH then
			BeginEvent( sceneId )
				AddText( sceneId, "Ngß½i không có ðü #G"..NeedCH.."#W ði¬m #GC¯ng Hiªn #Wð¬ hoàn t¤t nhi®m vø Nghiên CÑu!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			CityChangeAttr( sceneId, selfId, GUILD_CONTRIB_POINT, -NeedCH )
	...
	if GetNumText() == 1003 then
		if GetGuildPos(sceneId, selfId) <= 8 then
			BeginEvent( sceneId )
				AddText( sceneId, "Chï có Bang Chü ho£c Phó Bang Chü khä døng cho chÑc nång này!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
			return
		end
		...
		if guildid ~= cityguildid then
			BeginEvent(sceneId)
				strText = "Các hÕ không phäi là ngß¶i cüa bang hµi."
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
		...
		if guildid ~= 2 and guildid ~= 9 and guildid ~= 6 then
			BeginEvent(sceneId)
				strText = "Bang hµi cüa các hÕ không khä døng cho chÑc nång này."
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
		...
		if LuaFnGetPropertyBagSpace(sceneId,selfId) < 1 then
			BeginEvent(sceneId)
				strText = "Yêu c¥u 1 ô tr¯ng trong túi ðÕo cø!"
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
		...
		if guildid == 2 then
			...
			LuaFnSetItemCreator(sceneId,selfId,Cloth,"#GÐ® Nh¤t Bang Hµi - Nh¤t Ph¦m Chi Bào")
		elseif guildid == 9 then
			local Cloth = TryRecieveItem(sceneId,selfId,10310022,1)
			LuaFnSetItemCreator(sceneId,selfId,Cloth,"#GThiên C½ Lão T±")
		elseif guildid == 6 then
			local Cloth = TryRecieveItem(sceneId,selfId,10310023,1)
			LuaFnSetItemCreator(sceneId,selfId,Cloth,"#GTh¥n Ð Giáo Chü")
		end
		
		BeginEvent(sceneId)
			strText = "SØ c¯ g¡ng không ng×ng nghî ðã ðßþc ð«n ðáp xÑng ðáng!"
			AddText(sceneId,strText);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	end
