...
function x805028_UpdateEventList( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		...
		if IsHaveMission( sceneId, selfId, 1109 ) == 1 then
			AddNumText( sceneId, x805028_g_scriptId, "#GHoàn T¤t Nhi®m Vø Qu¯c Phòng", 4, 1001 )
		end
...
function x805028_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 1001 then
		if IsHaveMission( sceneId, selfId, 1109 ) <= 0 then --Chua Nhan Nhiem Vu
			BeginEvent( sceneId )
				AddText( sceneId, "Các hÕ vçn chßa nh§n Nhi®m vø Qu¯c Phòng nào cä?" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			...
			if misIndexComplete >= 1 then
				...
				BeginEvent( sceneId )
					...
					AddNumText( sceneId, x805028_g_scriptId, "Ta dùng #GNgân Lßþng", 6, 10011 )
					AddNumText( sceneId, x805028_g_scriptId, "Ta dùng #GC¯ng Hiªn", 6, 10012 )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
			end
		end
	...
	if GetNumText() == 10011 then
		...
		if CostMoney(sceneId,selfId,NeedMoney) == -1 then
			BeginEvent( sceneId )
				AddText( sceneId, "Ngß½i không có ðü #{_MONEY"..NeedMoney.."} ð¬ hoàn t¤t nhi®m vø Qu¯c Phòng!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			local misIndex = GetMissionIndexByID(sceneId, selfId, 1109)
	...
	if GetNumText() == 10012 then
		...
		if NeedCH > CurrCH then
			BeginEvent( sceneId )
				AddText( sceneId, "Ngß½i không có ðü #G"..NeedCH.."#W ði¬m #GC¯ng Hiªn #Wð¬ hoàn t¤t nhi®m vø Qu¯c Phòng!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		else
			...
			BeginEvent( sceneId )
				AddText( sceneId, "Các hÕ ðã hoàn t¤t nhi®m vø Qu¯c Phòng!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
			LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 148, 0)
		end
