...
--½Å±¾ºÅ
x332004_g_scriptId = 332004 --ÁÙÊ±Ğ´Cái này ,ÕæÕıÓÃtoÕ ğµ Ê±ºòmµt ¶¨Òª¸Ä.

--C¥n toÕ ğµ µÈc¤p

--Ğ§¹ûtoÕ ğµ ID
x332004_g_Impact1 = 3004 --ÁÙÊ±Ğ´Cái này 
x332004_g_Impact2 = -1 --²»ÓÃ

--**********************************
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x332004_OnDefaultEvent( sceneId, selfId, bagIndex )
...
--**********************************
function x332004_IsSkillLikeScript( sceneId, selfId)
...
--**********************************
function x332004_CancelImpacts( sceneId, selfId )
...
--**********************************
function x332004_OnConditionCheck( sceneId, selfId )
...
--**********************************
function x332004_OnDeplete( sceneId, selfId )
	
	local szTransfer;
	
	local szPlayerName = LuaFnGetName( sceneId, selfId );
	
	local nItemBagIndex = GetBagPosByItemSn(sceneId, selfId, 30505148);
	if nItemBagIndex >= 0 then		
		szTransfer = GetBagItemTransfer(sceneId,selfId, nItemBagIndex);
	end

	local message = format("#{_INFOUSR%s} u¯ng mµt viên thiên hÕ ğ® nh¤t kÏ dßşc#{_INFOMSG%s}, b¤t giác cäm giác toàn thân thß thái, cäm th¤y công lñc cüa mình có tiªn tri¬n l¾n.", szPlayerName, szTransfer );	
	AddGlobalCountNews( sceneId, message )

	local ExpBonus = 180000;
...
--**********************************
function x332004_OnActivateOnce( sceneId, selfId )
	if(-1~=x332004_g_Impact1) then
	--	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x332004_g_Impact1, 0);
	end
	return 1;
end
...
--×¢: ÕâÀïĞúng¼¼ÄÜÉúĞ§mµt ´ÎtoÕ ğµ Èë¿Ú
--**********************************
function x332004_OnActivateEachTick( sceneId, selfId)
...
--ĞÑÄ¿ÌáÊ¾
--**********************************
function x332004_NotifyTip( sceneId, selfId, msg )
