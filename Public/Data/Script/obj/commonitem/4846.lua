...
x334846_g_Impact_NotTransportList = { 200 } -- ½ûÖ¹´«ËÍµÄImpact
x334846_g_TalkInfo_NotTransportList = { "Trong trÕng thái ĞoÕt bäo không th¬ sØ døng ğ¸nh v¸ phù!" } -- ½ûÖ¹´«ËÍµÄImpactÌáÊ¾ĞÅÏ¢
...
function x334846_OnConditionCheck( sceneId, selfId )
	...
	if IsHaveMission( sceneId, selfId, 4021 ) > 0 then
		x334846_MsgBox( sceneId, selfId, "Ngß½i ğang · trong trÕng thái không th¬ truy«n tin, không th¬ sØ døng ğ¸nh vì phù!" )
		return 0
	end
	...
	if GetItemCount(sceneId, selfId, x334846_g_Yinpiao) >= 1  then
		x334846_MsgBox(sceneId, selfId, "Ngß½i ğang · trÕng thái chÕy hàng, không thª sØ døng kı hi®u ğ¸nh v¸")
		return 0
	end
	...
	if opx <= 0 and opy <= 0 then
		...
		if sceneId ~= SCENE_DALI and sceneId ~= SCENE_LUOYANG and sceneId ~= SCENE_SUZHOU and sceneId ~= SCENE_LOULAN then	--Â¥À¼Ò²ÊÇÖ÷³Ç--modify by xindefeng
			x334846_MsgBox( sceneId, selfId, "NgoÕi tr× Tô Châu, LÕc Dß½ng, ĞÕi Lı, Lâu Lan ra, không ğßşc sØ døng Ğ¸nh V¸ Phù trong các cänh khác!" )
			return 0
		end		
	else	
		...
		if (osid == SCENE_LOULAN) and (GetLevel(sceneId, selfId) < x334846_g_LimitTransLevel) then
			local szMsg = format("Lâu Lan c¥n c¤p %d m¾i có th¬ tiªn vào", x334846_g_LimitTransLevel)
