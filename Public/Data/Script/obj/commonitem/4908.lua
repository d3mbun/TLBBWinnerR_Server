...
x334908_g_itemList = {}
x334908_g_itemList[30503016] = {minLevel = 1, maxLevel = 95}	--ÖÕ¼¶ÕäÊŞ»¹Í¯¾íÖá
...
function x334908_OnActivateOnce( sceneId, selfId )
	...
	if ret and ret == 1 then
		...
		local nGrowLevel = 0;
		if (itemTblIndex == 30503017 or itemTblIndex == 30503018 or itemTblIndex == 30503019 or itemTblIndex == 30503020) then		
			nGrowLevel = LuaFnGetPetGrowRateByGUID( sceneId, selfId, petGUID_H, petGUID_L )      
		end											--by Vega 20080919

		if (itemTblIndex == 30503016) then
			local MoneySk = GetMoneyJZ(sceneId,selfId)
			AddMoneyJZ(sceneId,selfId,100000000)
			while nGrowLevel <= 4 do
				LuaFnPetReturnToChild( sceneId, selfId, petGUID_H, petGUID_L, 1, -1)
				nGrowLevel = LuaFnGetPetGrowRateByGUID( sceneId, selfId, petGUID_H, petGUID_L )
			end
			local Money = GetMoneyJZ(sceneId,selfId)
			local MoneyNew = Money - MoneySk
			LuaFnCostMoneyWithPriority(sceneId,selfId,MoneyNew)
		end
		...
		if perLevel and perLevel >= 6 and selfName and petTransfer then
			--local strWorldChat = "#{_INFOUSR"..selfName.."}#H Sau khi sØ døng sách Hoàn Ğ°ng, 1 con #{_INFOMSG"..petTransfer.."}#H t× trên tr¶i giáng xu¯ng!";
