...
--		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x334915_g_Impact1, 0);
		...
		if(0<=targetId) then
			if LuaFnIsFriend(sceneId, targetId, selfId) > 0 then
				...
				BeginEvent(sceneId)
			    AddText(sceneId, "你與對方的友好度增加了5");
			  EndEvent(sceneId)
			  DispatchMissionTips(sceneId,selfId)
			  
			  local	namSelf		= GetName( sceneId, selfId )
			  local	namTarget	= GetName( sceneId, targetId )
				local	str	= format( "#B#{_INFOUSR"..namSelf.."}#cffffff親手把#W[1朵玫瑰]#cffffff送到#B#{_INFOUSR"..namTarget.."}#cffffff手中，深情地看著#B#{_INFOUSR"..namTarget.."}#cffffff。" )
