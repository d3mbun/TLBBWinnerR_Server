...
--脚本号
x334917_g_scriptId = 334917 --临时写这个,真正用的时候一定要改.
...
--AE的目标关系标记
x334917_g_standFlag = 1 -- 2:队友， 1：友军， -1：敌军
--AE影响数目限制
x334917_g_effectCount = 4 -- -1:不限制
--效果的ID
x334917_g_Impact1 = 4917 --临时写这个
x334917_g_Impact2 = -1 --不用
...
--		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x334917_g_Impact1, 0);
		...
		if(0<=targetId) then
			if LuaFnIsFriend(sceneId, targetId, selfId) > 0 then
				...
				BeginEvent(sceneId)
			    AddText(sceneId, "你與對方的友好度增加了500");
			  EndEvent(sceneId)
			  DispatchMissionTips(sceneId,selfId)
			  
			  local	namSelf		= GetName( sceneId, selfId )
			  local	namTarget	= GetName( sceneId, targetId )
				local	str	= format( "#B#{_INFOUSR"..namSelf.."}#g7ffff0親手把#g000000#W[99朵玫瑰]#g7ffff0送到#g000000#B#{_INFOUSR"..namTarget.."}#g7ffff0手中，深情地看著#g000000#B#{_INFOUSR"..namTarget.."}#g7ffff0。" )
