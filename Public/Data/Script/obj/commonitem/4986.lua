...
--½Å±¾ºÅ
 x334986_g_scriptId = 334986 --ÁÙÊ±Ð´Õâ¸ö,ÕæÕýÓÃµÄÊ±ºòÒ»¶¨Òª¸Ä.
...
--×¢£ºÁ½¸öÆïÊõ»áÒ»¸ö¾Í¿ÉÒÔÆïÁË
 x334986_g_Equitation1 = 428 --ÐèÒªµÄÆïÊõ
 x334986_g_Equitation2 = -1 --ÐèÒªµÄÆïÊõ
--Ð§¹ûµÄID
 x334986_g_Impact1 = 4986 --ÁÙÊ±Ð´Õâ¸ö
 x334986_g_Impact2 = -1 --²»ÓÃ

--**********************************
--ÊÂ¼þ½»»¥Èë¿Ú
--**********************************
function  x334986_OnDefaultEvent( sceneId, selfId, bagIndex )
...
--**********************************
function  x334986_IsSkillLikeScript( sceneId, selfId)
...
--**********************************
function  x334986_CancelImpacts( sceneId, selfId )
	nRet = 0;
	nRet = LuaFnCancelSpecificImpact(sceneId, selfId,  x334986_g_Impact1)
	if(0<nRet) then
		return 1;
	end
	return 0;
...
--**********************************
function  x334986_OnConditionCheck( sceneId, selfId )
	--Ð£ÑéÊ¹ÓÃµÄÎïÆ·
	--if(1~=LuaFnVerifyUsedItem(sceneId, selfId)) then
		--return 0
	--end
	--Á½¸öÆïÊõ£¬»áÒ»¸ö¾Í¿ÉÒÔÁË
	if(-1~= x334986_g_Equitation1) then
		if(1==LuaFnHaveSpecificEquitation(sceneId, selfId,  x334986_g_Equitation1)) then
			return 1
		end
	end
	if(-1~= x334986_g_Equitation2) then
		if(1==LuaFnHaveSpecificEquitation(sceneId, selfId,  x334986_g_Equitation2)) then
			return 1
		end
	end
...
--**********************************
function  x334986_OnDeplete( sceneId, selfId )
	return 1; --Æï³Ë²»ÏûºÄ
end
...
--**********************************
function  x334986_OnActivateOnce( sceneId, selfId )
	if(-1~= x334986_g_Impact1) then
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId,  x334986_g_Impact1, 0)
	end
...
--×¢£ºÕâÀïÊÇ¼¼ÄÜÉúÐ§Ò»´ÎµÄÈë¿Ú
--**********************************
function  x334986_OnActivateEachTick( sceneId, selfId)
