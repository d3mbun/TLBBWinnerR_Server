...
function x400958_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetLevel(sceneId, selfId) < 10 then
		BeginEvent( sceneId )
			AddText( sceneId, "  ÐÆng c¤p chßa ðü 10 không th¬ d¸ch chuy¬n ðªn thành th¸ khác" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )	
		return	
	end
	...
	if GetTeamId(sceneId,selfId)>=0 and 
		...
		BeginEvent( sceneId )
			AddText( sceneId, "  Xin thÑ l²i vì ðß¾ng xá quá xa, nång lñc cüa chúng tôi hÕn chª vì v§y không ch¤p nh§n ðµi làm nhi®m vø v§n chuy¬n thß½ng nhân, vui lòng ð¬ lÕi ðµi thß½ng nhân và d¸ch chuy¬n mµt mình!" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	end
	...
	if id >=1 and id <= 6 then
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x400958_g_ScriptId, "Hüy", 0, 100);
		EndEvent()
		DispatchEventList(sceneId, selfId, targetId);
	elseif id == 100 then
