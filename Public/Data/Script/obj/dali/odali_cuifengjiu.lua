...
function x002026_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		if GetLevel( sceneId, selfId ) >= 10 then
			...
			AddNumText( sceneId, x002026_g_ScriptId, "Ðªn ta ðªn nhæng môn phái khác", 9, 1012 )
			--for i, mp in x002026_g_mpInfo do
			--	AddNumText( sceneId, x002026_g_ScriptId, "ÃÅÅÉ - "..mp[1], 9, i )
			--end
		else
			AddText( sceneId, "  Các hÕ c¥n ðÕt ðªn c¤p 10 tr· lên m¾i có th¬ ðªn các thành th¸ khác." )
	...
	if GetTeamId(sceneId,selfId)>=0 and 
		...
		for	i=0,num-1 do
			...
			if IsHaveMission(sceneId,mems[i],4021) > 0 then
				x002026_MsgBox( sceneId, selfId, targetId, "  Trong ðµi ngû cüa các hÕ có ngß¶i ch· hàng b¢ng ðß¶ng thuÖ, d¸ch trÕm chúng ta không th¬ cung c¤p d¸ch vø cho các hÕ" )
				return
			end
		end
	end
