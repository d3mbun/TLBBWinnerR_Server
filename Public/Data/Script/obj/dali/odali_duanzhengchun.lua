...
function x002004_UpdateEventList( sceneId, selfId,targetId )
	...
	BeginEvent(sceneId)	
		...
		if missionIndex ~= -1 then
			...
			if missionName ~= "Nhi®m vø Ðoàn Chính Thu¥n" and IsMissionHaveDone( sceneId, selfId, x002004_g_ActivateMissionId ) > 0 then
				AddNumText( sceneId, x002004_g_scriptId, "Nhi®m vø Ðoàn Chính Thu¥n", 3, 1 )
			end
...
function x002004_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then										-- ¶ÎÓþÈÎÎñ
		if IsHaveMission( sceneId, selfId, x002004_g_RSMissionId ) > 0 then
			x002004_NotifyFailBox( sceneId, selfId, targetId, "    Các hÕ còn có nhi®m vø quan h® v¾i các anh hùng (Nhi®m Vø k¸ch tình tu¥n hoàn) chßa hoàn thành, sau khi hoàn thành hãy ðªn tìm ta." )
			return
		end
		...
		--¿´ÏÂ½ñÌìµÄ¶ÎÓþÈÎÎñÊÇ²»ÊÇ×öÍê50´ÎÁË
