...
function x015001_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText()==0	then
		...
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x015001_g_scriptId, "Ta mu¯n bái nh§p phái Nga My",6,3)
	...
	if GetNumText()==3	then
		...
		elseif GetLevel( sceneId, selfId ) < 10 then
			...
			elseif x015001_g_MenPai==9	then
				...
				x015001_MsgBox( sceneId, selfId, "L¤y ðßþc 20 t¤m Chiêu t§p l®nh cüa môn phái" )
	...
	if GetNumText()==6 then
		BeginEvent(sceneId)
			AddText(sceneId, "Thôi Løc Hoa [98,51] có th¬ dÕy ngß½i kÛ nång chiªn ð¤u cüa phái ta, ông ta · ngay bên cÕnh ")
		EndEvent(sceneId)
		DispatchEventList(sceneId, selfId, targetId)
		CallScriptFunction( SCENE_SCRIPT_ID, "AskTheWay", sceneId, selfId, sceneId, 98, 51, "´Þ¾GÈA" )
		return
	end
end
