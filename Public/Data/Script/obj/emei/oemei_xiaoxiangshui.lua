...
function  x015007_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		if	GetMenPai( sceneId, selfId) == 4 then
			...
			if	(0==LuaFnHaveSpecificEquitation(  sceneId, selfId, 1084))	then
				AddNumText(sceneId, x015007_g_ScriptId,"H÷c kÜ thu§t: Lßu Ly Phøng",12,80)
			end
		end
...
function  x015007_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 80 then
		BeginEvent( sceneId )
			AddText( sceneId, "#{MPZQ_xml_XX(02)}#{MPZQ_xml_XX(08)}" )
			AddText( sceneId, "#{MPZQ_xml_XX(03)}" )
			AddNumText(sceneId, x015007_g_ScriptId,"Có",-1,2)
		AddNumText(sceneId, x015007_g_ScriptId,"Không",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
