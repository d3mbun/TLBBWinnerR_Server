...
function  x010010_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		if	GetMenPai( sceneId, selfId) == 2 then
			...
			if	(0==LuaFnHaveSpecificEquitation(  sceneId, selfId, 1085))	then
				AddNumText(sceneId, x010010_g_ScriptId,"H÷c kÜ thu§t: Ngân Nguy®t Lang",12,80)
			end
		end
	EndEvent(sceneId)
...
function  x010010_OnEventRequest( sceneId, selfId, targetId, eventId )
	if GetNumText() == 40 then
		BeginEvent( sceneId )
			...
			AddNumText(sceneId, x010010_g_ScriptId,"Duy®t",-1,0)
      AddNumText(sceneId, x010010_g_ScriptId,"Hüy",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	end
	...
	if GetNumText() == 60 then
		BeginEvent( sceneId )
			...
			AddNumText(sceneId, x010010_g_ScriptId,"Duy®t",-1,1)
		AddNumText(sceneId, x010010_g_ScriptId,"Hüy",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	...
	if GetNumText() == 80 then
		BeginEvent( sceneId )
			AddText( sceneId, "#{MPZQ_xml_XX(02)}#{MPZQ_xml_XX(06)}" )
			AddText( sceneId, "#{MPZQ_xml_XX(03)}" )
			AddNumText(sceneId, x010010_g_ScriptId,"Duy®t",-1,2)
		AddNumText(sceneId, x010010_g_ScriptId,"Hüy",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
