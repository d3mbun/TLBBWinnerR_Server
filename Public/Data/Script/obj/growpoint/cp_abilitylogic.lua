...
function x701601_ReomveProc(sceneId,selfId, EquipIndex, GemIndex, MatIndex)
	local EquipID = LuaFnGetItemTableIndexByIndex( sceneId, selfId, EquipIndex )
	if EquipID >= 10309001 and EquipID <= 10309015 then
		BeginEvent( sceneId )
			AddText( sceneId, "Trang b¸ Tân Thü, không th¬ tháo ng÷c!" )
		EndEvent( sceneId )
		DispatchMissionTips( sceneId, selfId )
		return 1
	end
	
	if EquipID == 10305444 or EquipID == 10304426 or EquipID == 10302448 or EquipID == 10300422 then
		BeginEvent( sceneId )
			AddText( sceneId, "Trang b¸ Tân Thü, không th¬ tháo ng÷c!" )
		EndEvent( sceneId )
		DispatchMissionTips( sceneId, selfId )
...
function x701601_SplitGemSucceed(sceneId,selfId, EquipPos, GemPos, MatPos)
    ...
    if (CharmId >= 30900038 and CharmId <= 30900044) then  --¸ß¼¶±¦Ê¯Õª³ý·û
	     ...
	     str = format( "#H#{_INFOUSR%s} c¥m #{_INFOMSG%s} mi®ng l¦m b¦m vài câu th¥n chú, chï th¤y #{_INFOMSG%s} t× t× hi®n ra nhßng không h« b¸ vÞ, ðúng là cñc ph¦m chi phù!", PlayerName, MatInfo, GemInfo)
