...
function	 x712001_OnOpen(sceneId,selfId,targetId)
	...
	x712001_g_FishTime = random(80000)+20000	--»ñµÃÒ»¸ö20-100ÃëÖ®¼äµÄËæ»úÊı
	--x712001_g_FishTime = random(8000)+2000	--»ñµÃÒ»¸ö2-10ÃëÖ®¼äµÄËæ»úÊı
	SetAbilityOperaTime(sceneId,selfId,x712001_g_FishTime)	--ÉèÖÃÒ»´Îµöµ½ÓãµÄÊ±¼ä
...
function	x712001_OnProcOver(sceneId,selfId,targetId)
	...
	ret_1 = TryRecieveItem(sceneId,selfId,x712001_g_MainItemId,QUALITY_MUST_BE_CHANGE)
	if ret_1 > 0 then					-->0±íÊ¾ÎïÆ·³É¹¦·ÅÈë±³°üÖĞ
		Msg2Player(sceneId,selfId,"Các hÕ ğã câu ğßşc mµt con Thäo Ngß",MSG2PLAYER_PARA)
		-- Ôö¼ÓÊìÁ·¶È
		ABilityID	=	GetItemBoxRequireAbilityID(sceneId,targetId)
