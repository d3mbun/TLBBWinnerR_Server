...
--Éú³¤µãÐèÒªµÄµÈ¼¶
x711050_g_ItemBoxNeedLevel = 5

function	x711050_OnOpen(sceneId,selfId,targetId)
	--µÃµ½Éú³¤µãµÄÖ÷ÈËGUID
	...
	if AbilityLevel >= x711050_g_ItemBoxNeedLevel then
		return OR_OK
	else
		return OR_NO_LEVEL
	end
...
function	x711050_OnProcOver(sceneId,selfId,targetId)
	return OR_OK
end

function	 x711050_OnRecycle(sceneId,selfId,targetId)
	local num = 0
	--È¡µÃÉú³¤µãµÄ×ø±ê
