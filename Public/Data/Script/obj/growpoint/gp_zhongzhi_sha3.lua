...
--Éú³¤µãÐèÒªµÄµÈ¼¶
x711068_g_ItemBoxNeedLevel = 11

function x711068_OnOpen(sceneId,selfId,targetId)
	--µÃµ½Éú³¤µãµÄÖ÷ÈËGUID
	...
	if ItemBoxOwnerGUID~= PlayerGuid then
		BeginEvent(sceneId)
	  		AddText(sceneId,"Tùy ti®n thu lúa gÕo cüa ngß¶i khác là không ðßþc!");
	  	EndEvent(sceneId)
	...
	if AbilityLevel >= x711068_g_ItemBoxNeedLevel then
		return OR_OK
	else
		return OR_NO_LEVEL
	end
...
function	x711068_OnProcOver(sceneId,selfId,targetId)
	return OR_OK
end

function	 x711068_OnRecycle(sceneId,selfId,targetId)
	local num = 0
	--È¡µÃÉú³¤µãµÄ×ø±ê
