...
local	key	= GetNumText()
	...
	if key == 1001 then	--È·ÈÏÒªÖÎÁÆ
		...
		--Ç®²»¹»	
			...
			BeginEvent( sceneId )
				AddText( sceneId, "  Ngß½i không ğü ngân lßşng!" )
				EndEvent( sceneId )
	...
	if key == 0 then
		if GetHp( sceneId, selfId ) == GetMaxHp( sceneId, selfId ) and
			...
			BeginEvent( sceneId )
			AddText( sceneId, "  Ngß½i r¤t khöe mÕnh, không c¥n tr¸ li®u!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		...
		if gld <= 0 then
			...
			BeginEvent( sceneId )
				AddText( sceneId, "  Các hÕ c¥n chi #G#{_EXCHG"..gld.."}#W ğ¬ khôi phøc khí huyªt, xác nh§n tr¸ li®u?" )
				AddNumText( sceneId, x125004_g_scriptId, "Duy®t", -1, 1001 )
				AddNumText( sceneId, x125004_g_scriptId, "Hüy", -1, 1000 )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
		end
	else
...
function x125004_Restore_hpmp( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		AddText( sceneId, "Khí huyªt cüa bÕn ğã ğßşc phøc h°i." );
	EndEvent( sceneId )
	DispatchMissionTips( sceneId, selfId )
