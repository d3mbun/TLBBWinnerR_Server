...
function x155000_OnEventRequest(sceneId, selfId, targetId, eventId)
	...
	elseif eventId == x155000_g_eventId_done then	--Ôö¼ÓÈ·ÈÏÒ³,·ÀÖ¹Íæ¼Òµã´íÁË--modify by xindefeng		
		BeginEvent( sceneId )
			AddText( sceneId, "    Các hÕ có ch¡c mu¯n kªt thúc Hôn l­ chßa? các hÕ s¨ không th¬ quay lÕi l­ dß½ng ðßþc næa!" )
			AddNumText(sceneId, x155000_g_ScriptId, "Có, tÕi hÕ ch¡c mu¯n kªt thúc hôn l­!", 8, 100 )
			AddNumText(sceneId, x155000_g_ScriptId, "TÕi hÕ không có vi®c gì..", 8, 101 )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
...
function x155000_Redeem(sceneId, selfId, targetId)
	...
	if haveInvitation == 1 then
								...
								if randGemValue < gemItem.rate then
									...
									if addItemRet and addItemRet == 1 then
										...
										if selfName and itemCreatorName and gemName and gemTransfer then
											BroadMsgByChatPipe(sceneId, selfId, "#W#{_INFOUSR"..selfName.."}#I TÕi #W#{_INFOUSR"..itemCreatorName.."}#I trong Hôn L­, ðã dùng thi®p m¶i ð±i ðßþc 1 viên#W#{_INFOMSG"..gemTransfer.."}#I.", 4);
		...
		x155000_MessageBox(sceneId, selfId, targetId, "Xin thÑ l²i, các hÕ không có thi®p m¶i nên không th¬ ð±i ðßþc!");
	end
end
