...
function x402106_OnDefaultEvent( sceneId, selfId,targetId )
	...
	BeginEvent(sceneId)
		...
		if sceneId == 1 then
			...
			AddNumText(sceneId,x402106_g_ScriptId,"Vào c± mµ",9,1)

		elseif sceneId == 194 then

			AddText(sceneId,"#{HANYUBED_20071221_02}")
			AddNumText(sceneId,x402106_g_ScriptId,"Tr· v« Tô Châu",9,2)

		end

		AddNumText(sceneId,x402106_g_ScriptId,"Mua C± Mµ hành công yªu quyªt",6,3)
		AddNumText(sceneId,x402106_g_ScriptId,"4 ði¬m c¥n chú ý khi tu hành · Hàn Ng÷c C¯c",11,4)
...
function x402106_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif NumText == 3 then
		...
		BeginEvent(sceneId)
			...
			AddNumText(sceneId,x402106_g_ScriptId,"Duy®t",8,5)
			AddNumText(sceneId,x402106_g_ScriptId,"Hüy",8,6)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
...
function x402106_BuyBook( sceneId, selfId, targetId )
	...
	--ÌáÊ¾....
	local str1 = "BÕn phäi chi #{_MONEY"..NeedMoney.."}."
	local str2 = "BÕn mua mµt #{_ITEM"..(30700200).."}."
	BeginEvent( sceneId )
