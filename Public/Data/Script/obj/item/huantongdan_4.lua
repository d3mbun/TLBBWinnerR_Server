...
function x300100_OnActivateOnce( sceneId, selfId )
	...
	if (itemTblIndex == 30309186) or (itemTblIndex == 30309187) then --Èç¹ûĞúng»¹Í¯µ¤: Ô§Ñì
		strText = format("#{_INFOUSR%s} #{HT11}%s #{HT12} #{_INFOMSG%s}", PlayerName, GetItemName(sceneId,itemTblIndex), PetName);	
	else
		local inforand = random(3)		
		if inforand == 1 then
			strText = format("#{HT01}%s #{HT02} #{_INFOMSG%s} #{HT03} #W%s #{HT04}", GetItemName(sceneId,itemTblIndex), PetName, PlayerName )
		elseif inforand == 2 then
			strText = format("#{HT05} #{_INFOUSR%s}#{HT06} %s #{HT07} #{_INFOMSG%s}", PlayerName, GetItemName(sceneId,itemTblIndex), PetName )
		else
			strText = format("#{_INFOUSR%s}#{HT08} %s #{HT09} #{_INFOMSG%s}", PlayerName, GetItemName(sceneId,itemTblIndex), PetName )
		end
	end
