...
function x005001_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then
		if IsHaveMission( sceneId, selfId, x005001_g_RSMissionId ) > 0 then
			x005001_NotifyFailBox( sceneId, selfId, targetId, "    Các hÕ còn có nhi®m vø quan h® v¾i các anh hùng (Nhi®m Vø k¸ch tình tu¥n hoàn) chßa hoàn thành, sau khi hoàn thành hãy ðªn tìm ta." )
			return
		end
...
function x005001_NotifyMessage(sceneId, selfId,targetId )		
		...
		if(nCount == 10 or nCount == 20 or nCount == 30 or nCount == 40 )	then				
				local msg = format("[%s]: #W#{_INFOUSR%s} #GA Châu #P#{JINGHU_RUANXINZHU}", GetSceneName(sceneId), GetName(sceneId, selfId) )
				LuaFnNpcChat(sceneId, targetId, 1, msg)				
		end
		--Èç¹ûÃ¿Íê³É50´Î·¢²¼ÊÀ½çÏûÏ¢
		if(nCount == 50)	then								
				local msg = format("#W#{_INFOUSR%s} #GA Châu #P#{JINGHU_RUANXINZHU}", GetName(sceneId, selfId) )
...
function x005001_GiveRabbitEar( sceneId, selfId )
	...
	if ret and ret > 0 then
		...
		if itemTransfer and selfName then
			local msg = "#YNguy­n Tinh Trúc: #W#{_INFOUSR"..selfName.."}#P#{tuerduo}#{_INFOMSG"..itemTransfer.."} #Pmang vào #W#{_INFOUSR"..selfName.."} #Ptrên ð¥u bi¬u hi®n lòng cäm tÕ!";
...
function x005001_GiveSmallRabbitEar( sceneId, selfId )
	...
	if ret and ret > 0 then
		...
		if itemTransfer and selfName then
			local msg = "#YNguy­n Tinh Trúc: #W#{_INFOUSR"..selfName.."}#P#{tuerduo2}#{_INFOMSG"..itemTransfer.."} #Pðßa cho #W#{_INFOUSR"..selfName.."}#P#{tuerduo3}";
			BroadMsgByChatPipe(sceneId, selfId, msg, 4);
		end
	end
