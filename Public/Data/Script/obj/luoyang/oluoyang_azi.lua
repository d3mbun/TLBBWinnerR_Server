...
function x000016_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then										-- Nhi®m vø A TØ
		if IsHaveMission( sceneId, selfId, x000016_g_RSMissionId ) > 0 then
			x000016_NotifyFailBox( sceneId, selfId, targetId, "    Các hÕ còn có nhi®m vø quan h® v¾i các anh hùng (Nhi®m Vø k¸ch tình tu¥n hoàn) chßa hoàn thành, sau khi hoàn thành hãy ðªn tìm ta." )
			return
		end
