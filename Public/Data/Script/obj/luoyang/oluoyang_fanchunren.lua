...
function x000030_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		if GetHumanGuildID(sceneId, selfId) == -1 then
			AddNumText(sceneId,x000030_g_scriptId,"#GThoát ly Tâm Hoài C¯ Chü",4,999)	
		end
...
function x000030_OnEventRequest( sceneId, selfId, targetId, eventId )
	if GetNumText() == 999 then
		if LuaFnHaveImpactOfSpecificDataIndex(sceneId, selfId, 59) == 1 then
			...
			BeginEvent(sceneId)	
				AddText( sceneId, "Các hÕ không còn trÕng thái #GTâm Hoài C¯ Chü" )		
				AddText( sceneId, "Nªu vçn không th¬ gia nh§p bang hµi, hãy ðång nh§p lÕi trò ch½i!" )		
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		else
			BeginEvent(sceneId)	
