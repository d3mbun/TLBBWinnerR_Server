...
function x000077_OnDefaultEvent( sceneId, selfId,targetId )
	...
	BeginEvent(sceneId)
		...
		--AddNumText--( sceneId, x000077_g_ControlScript, "Dùng Bäo ThÕch toái phiªn ð±i Bäo ThÕch c¤p 4", 6, 100 )
		--AddNumText--( sceneId, x000077_g_ControlScript, "Dùng Bäo ThÕch toái phiªn ð±i Bäo ThÕch c¤p 5", 6, 200 )
		--AddNumText--( sceneId, x000077_g_ControlScript, "Dùng Bäo ThÕch toái phiªn ð±i Bäo ThÕch c¤p 6", 6, 300 )
		--AddNumText--( sceneId, x000077_g_ControlScript, "Dùng Bäo ThÕch toái phiªn ð±i Bäo ThÕch c¤p 7", 6, 400 )
	  ...
	  if CallScriptFunction( x000077_g_ControlScript, "CheckRightTime", sceneId) == 1 then
		  ...
		  if	i<=0	 then
			  ...
			  AddText(sceneId,"Tß¾ng t¯t ðÕi phú ðÕi quý biªt bao! ngß¶i phúc d¥y nhß các hÕ ch¡c không ð¡n ðo mß¶i lÕng ti«n quë chÑ?")
		  end
		end		
...
function x000077_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if 100 == key or 200 == key or 300 == key or 400 == key then
		...
		AddNumText( sceneId, x000077_g_ControlScript, "Ð±i H± Nhãn ThÕch", 6, 1 + key )
		AddNumText( sceneId, x000077_g_ControlScript, "Ð±i Miêu Nhãn ThÕch", 6, 2 + key )
		AddNumText( sceneId, x000077_g_ControlScript, "Ð±i TØ Ng÷c", 6, 3 + key )
		AddNumText( sceneId, x000077_g_ControlScript, "Ð±i T± Mçu Løc", 6, 4 + key )
		AddNumText( sceneId, x000077_g_ControlScript, "Ð±i thu¥n t¸nh Lam tinh thÕch", 6, 5 + key )
		AddNumText( sceneId, x000077_g_ControlScript, "Ð±i thu¥n t¸nh H°ng tinh thÕch", 6, 6 + key )
		AddNumText( sceneId, x000077_g_ControlScript, "Ð±i thu¥n t¸nh Hoàng tinh thÕch", 6, 7 + key )
		AddNumText( sceneId, x000077_g_ControlScript, "Ð±i thu¥n t¸nh Løc tinh thÕch", 6, 8 + key )
		AddNumText( sceneId, x000077_g_ControlScript, "Trao ð±i Huyªt Tinh ThÕch", 6, 9 + key )
		AddNumText( sceneId, x000077_g_ControlScript, "Trao ð±i H°ng Bäo ThÕch", 6, 10 + key )
...
function x000077_OnMyChange( sceneId, selfId, targetId, key )
	...
	if LuaFnGetAvailableItemCount( sceneId, selfId, unt.NeedItm ) < unt.NeedNum then
		...
		local strMsg = format("Ð±i #H#{_ITEM%d}#W c¥n 72 cái #H#{_ITEM%d}#W, nguyên li®u cüa các hÕ không ðü.", unt.id, unt.NeedItm)
	...
	if EndAddItem( sceneId, selfId ) > 0 then
		if LuaFnDelAvailableItem( sceneId, selfId, unt.NeedItm, unt.NeedNum ) == 1 then
			...
			x000077_MsgBox( sceneId, selfId, targetId, "    Kh¤u tr× v§t ph¦m th¤t bÕi!" )
			return
		end
	else
		x000077_MsgBox( sceneId, selfId, targetId, "    Xin l²i, tay các hÕ ðã näi ð¥y, không th¬ ð±i." )
		return
	end
