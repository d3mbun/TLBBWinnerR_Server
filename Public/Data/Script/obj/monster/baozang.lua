...
function x502000_OnDie( sceneId, objId, killerId )
	...
	elseif namMonster == "Ðµc Ty Tri Thù Vß½ng" then
		namMonster = "#{_BOSS32}"
	elseif namMonster == "Tri«n t½ tri chu vß½ng" then
		namMonster = "#{_BOSS33}"
...
--		AddItemToBox(sceneId, itemBoxId, QUALITY_MUST_BE_CHANGE, 1, 40004414);
	...
	if sceneId == 123 or sceneId == 126 or sceneId == 148 or sceneId == 149 then
		...
		if randMessage == 1 then
	   		message = format("#G%s#cff99cc cüa #Y%s#cff99cc#{BroadMsg_YanWangGuMu_00}#W#{_INFOUSR%s} #cff99cc#{BroadMsg_YanWangGuMu_01} #Y%s#cff99cc#{BroadMsg_YanWangGuMu_02}", namScene, namMonster, namHuman, namMonster );
		elseif randMessage == 2 then		
