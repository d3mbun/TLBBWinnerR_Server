...
x400960_g_ScriptId = 400960; --½Å±¾ºÅ
x400960_g_name	="Xa Truy«n Tín";
...
x400960_g_transfer_target =
{
	...
	[3] = {x = 250, z = 105, scene_num = 2}, 		--ÐÕi Lý
	[4] = {x = 294, z = 90, scene_num = 186}, 	--Lâu Lan
	[5] = {x = 186, z = 43, scene_num = 28},		--Nam Häi
	[6] = {x = 158, z = 113, scene_num = 22}, 	--Trß¶ng BÕch S½n
}

-- ÊÕ·Ñ½ð¶î
x400960_g_transfer_cost = 5000; -- 50Òø½»×Ó
...
function x400960_UpdateEventList( sceneId, selfId, targetId )
	BeginEvent(sceneId);
		...
		AddNumText(sceneId, x400960_g_ScriptId, "LÕc Dß½ng", 9, 1);
		AddNumText(sceneId, x400960_g_ScriptId, "Tô Châu", 9, 2);
		AddNumText(sceneId, x400960_g_ScriptId, "ÐÕi Lý", 9, 3);
		AddNumText(sceneId, x400960_g_ScriptId, "Lâu Lan", 9, 4);
		AddNumText(sceneId, x400960_g_ScriptId, "Nam Häi", 9, 5);
		AddNumText(sceneId, x400960_g_ScriptId, "Trß¶ng BÕch S½n", 9, 6);
	EndEvent()
...
function x400960_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if id >=1 and id <= 6 then
		BeginEvent(sceneId)
			...
			if id == 1 then	
				AddNumText(sceneId, x400960_g_ScriptId, "Xác nh§n", 0, 11);
			elseif id == 2 then
				AddNumText(sceneId, x400960_g_ScriptId, "Xác nh§n", 0, 21);
			elseif id == 3 then
				AddNumText(sceneId, x400960_g_ScriptId, "Xác nh§n", 0, 31);
			elseif id == 4 then
				AddNumText(sceneId, x400960_g_ScriptId, "Xác nh§n", 0, 41);
			elseif id == 5 then
				AddNumText(sceneId, x400960_g_ScriptId, "Xác nh§n", 0, 51);
			elseif id == 6 then
				AddNumText(sceneId, x400960_g_ScriptId, "Xác nh§n", 0, 61);
			end
			AddNumText(sceneId, x400960_g_ScriptId, "Hüy bö", 0, 100);
		EndEvent()
		DispatchEventList(sceneId, selfId, targetId);
	elseif id == 100 then
