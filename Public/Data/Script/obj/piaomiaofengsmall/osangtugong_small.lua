...
function x402284_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		AddNumText( sceneId, x402284_g_ScriptId, "Khiêu chiªn", 10, 1 )
	EndEvent(sceneId)
...
function x402284_OnPMFTimer( sceneId, step, data1, data2 )
	...
	if 7 == step then
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Cuµc chiªn 5 giây sau s¨ b¡t ð¥u" )
		return
	end

	if 6 == step then
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Cuµc chiªn 4 giây sau s¨ b¡t ð¥u" )
		return
	end

	if 5 == step then
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Cuµc chiªn 3 giây sau s¨ b¡t ð¥u" )
		return
	end

	if 4 == step then
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Cuµc chiªn 2 giây sau s¨ b¡t ð¥u" )
		return
	end

	if 3 == step then
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Cuµc chiªn 1 giây sau s¨ b¡t ð¥u" )
		return
	end

	if 2 == step then
		--ÌáÊ¾Õ½¶·¿ªÊ¼....
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Cuµc chiªn b¡t ð¥u" )
		--É¾³ýNPC....
